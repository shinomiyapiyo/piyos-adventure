// ─────────────────────────────────────────────────────────────────────────────
// generate-skin-cyber-openai.mjs — 新スキン「サイバーぴよ」(タイトルショップ200万円) の素材生成。
// 能力: ドローンビット（丸型メカひよこが随伴し前後同時レーザー・コードは別途実装）。ここでは見た目だけ作る。
// 意匠: 白のサイバースーツ＋金の発光ライン＋金ヘッドセット(顔は見える)。白×金＝既存スキンと被らない近未来配色。
// ⚠ドローンは別スプライト（コード側で随伴描画）のため、キャラ画像には絶対に含めない。
// モーション: 歩行は後段で Veo（veo-samurai-motion.mjs を種プロンプト差し替えで流用・1本ずつ）。
//
// モード（samuraiスクリプトと同型）:
//   --anchor --n=4       アンカー立ち絵候補を生成（oai_base_side_2_1024.png の衣装差し替え）
//                        → _raw/cyber_anchor_<i>_1024.png（人間が選定）
//   --idle --anchorFile=cyber_anchor_2_1024.png
//                        確定アンカーを player_idle_v1.png に整列して images/skin_cyber_idle.png(64×64) をコミット
//   --frames --anchorFile=cyber_anchor_2_1024.png [--only=jump,fall]
//                        jump/fall を生成し player 基準に整列してコミット
// 実行: zsh -ic 'cd tools && node generate-skin-cyber-openai.mjs --anchor --n=4'
//   ※ OPENAI_API_KEY はログインシェル経由（zsh -ic）で読み込む
// 生成後の反映（Claude側で実施）: sprites.js に skin_cyber_* 登録（全コマ揃ってから）。
// ─────────────────────────────────────────────────────────────────────────────
...
const BASE     = path.join(RAW_DIR, 'oai_base_side_2_1024.png'); // 忍者/メイド/魔女/侍と同じ横向きベース種
const ANCHOR_FILE = getArg('anchorFile') || 'cyber_anchor_1_1024.png';

// サイバーぴよの意匠（アンカー・全ポーズ共通のidentity）。白×金＝クリーンな近未来スーツ。
// 顔はバイザーで隠さない（可愛さ優先・ユーザー要件）。緑は使わない。ドローンは絶対に描かない。
const CYBER_IDENTITY = [
  ...
  'replaced with a cute FUTURISTIC CYBER SUIT: a sleek WHITE form-fitting tech bodysuit with glowing GOLD accent',
  'lines (thin circuit-like trim on the chest, arms and legs), small WHITE gloves and WHITE boots with GOLD soles and',
  'gold trim, a small GOLD tech HEADSET over her ears (her face stays FULLY VISIBLE — no visor covering the eyes, no',
  'helmet), and a tiny compact WHITE thruster backpack unit with gold trim on her back. Her black hair is tied in a',
  'HIGH PONYTAIL with a small gold hair tie. Color palette is strictly WHITE + GOLD (+ black hair): absolutely NO',
  'green anywhere. She has NO drone, NO companion robot, NO floating object — the girl alone.',
...
const ANCHOR_OUTPUT = [
  'Same calm standing side view facing right (arms relaxed at her sides), single character only, same',
  '2-heads-tall scale and same position in frame as the input image. The background MUST be FULLY TRANSPARENT (alpha) —',
  'absolutely NO background, NO scenery, NO ground, NO shadow, NO glow effect, NO gradient, NO colored backdrop; only',
  'the character pixels on transparency. No text, no border.',
].join(' ');

// jump/fall 用（samurai方式: [アンカー, player_<pose>.png] の2枚渡し。2枚目はポーズのヒントのみ）
const FRAMES = [
  { key: 'jump', base: 'player_jump.png', motion: 'JUMPING UP pose facing RIGHT: clearly AIRBORNE, both knees bent and tucked up, feet off the floor, arms raised, a tiny gold spark from her backpack thruster. Her ponytail flutters upward. NOT a standing pose.' },
  { key: 'fall', base: 'player_fall.png', motion: 'FALLING pose facing RIGHT: clearly AIRBORNE and dropping, arms spread slightly out for balance, legs loose and trailing, ponytail streaming upward. NOT a standing pose.' },
];
const FRAME_OUTPUT = [
  'Output the SAME cyber-suit girl, re-posed into the TARGET POSE — one frame of a side-scrolling sprite animation.',
  'Identical face, headset, ponytail, suit, colors, line work, chibi proportions and art style as the first',
...
// ── ユーティリティ（samurai/witch/ninjaスクリプトと同一の整列ロジック） ──
...
async function main() {
  ...
  if (hasFlag('anchor')) {
    for (let i = 1; i <= N; i++) {
      ...
      const buf = await editImage([BASE], `${CYBER_IDENTITY}\n\n${ANCHOR_OUTPUT}`);
      await fs.writeFile(path.join(RAW_DIR, `cyber_anchor_${i}_1024.png`), buf);
      console.log(`  ✓ _raw/cyber_anchor_${i}_1024.png`);
    }
    console.log('\n次: 候補を人間が見て1枚選定 → --idle と --frames に --anchorFile=cyber_anchor_<選んだ番号>_1024.png を渡す');
    return;
  }
  if (hasFlag('idle')) {
    const buf = await fs.readFile(path.join(RAW_DIR, ANCHOR_FILE));
    const aligned = await alignToBase(buf, 'player_idle_v1.png');
    await fs.writeFile(path.join(IMAGES_DIR, 'skin_cyber_idle.png'), aligned);
    await fs.writeFile(path.join(RAW_DIR, 'cyber_idle_256.png'), await sharp(aligned).resize(256, 256, { kernel: 'nearest' }).png().toBuffer());
    console.log('✓ images/skin_cyber_idle.png (64×64)');
    return;
  }
  if (hasFlag('frames')) {
    ...
    for (const fr of targets) {
      ...
      await fs.writeFile(path.join(RAW_DIR, `cyber_${fr.key}_1024.png`), buf);
      const aligned = await alignToBase(buf, fr.base);
      await fs.writeFile(path.join(IMAGES_DIR, `skin_cyber_${fr.key}.png`), aligned);
      await fs.writeFile(path.join(RAW_DIR, `cyber_${fr.key}_256.png`), await sharp(aligned).resize(256, 256, { kernel: 'nearest' }).png().toBuffer());
      console.log(`  ✓ images/skin_cyber_${fr.key}.png`);
    }
    return;
  }
