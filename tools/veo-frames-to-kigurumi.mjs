// ─────────────────────────────────────────────────────────────────────────────
// veo-frames-to-kigurumi.mjs（ninja版のコピー） — Veoの緑背景フレームを walk スプライトに変換。
...
const FRAMES_DIR = path.join(RAW_DIR, getArg('dir') || 'veo_frames_kigurumi');   // --dir=veo_frames_kigurumi2 で2本目を使う
...
async function main(){
  // idle 基準
  const idle = await rawRGBA(await fs.readFile(path.join(IMAGES_DIR,'skin_kigurumi_idle.png')));
  ...
  for(let k=0;k<FRAMES.length;k++){
    ...
    await fs.writeFile(path.join(RAW_DIR,`veo_kigurumi_walk_${k+1}_norm_64.png`), norm);
    await fs.writeFile(path.join(RAW_DIR,`veo_kigurumi_walk_${k+1}_norm_256.png`), await sharp(norm).resize(256,256,{kernel:'nearest'}).png().toBuffer());
    console.log(`${key} <= f_${FRAMES[k]}  content=${bb.w}x${bb.h} S=${sv.S.toFixed(3)} V=${sv.V.toFixed(3)} -> ${tW}x${tH}`);
    if(COMMIT){ await fs.writeFile(path.join(IMAGES_DIR,`skin_kigurumi_${key}.png`), norm); console.log(`  ✓ images/skin_kigurumi_${key}.png`); }
  }
}
