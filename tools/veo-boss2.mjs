// ─────────────────────────────────────────────────────────────────────────────
// veo-boss2.mjs — 闇のカラス（空中ボス）のモーション動画を Veo で生成する。
// ルール: 動きの差分コマは必ず Veo 動画から切り出す（独立画像生成はモーションが崩れる）。
// 種画像 = 承認済み idle（_raw/boss2_idle_raw.png ＝ 高解像度の生画像）を緑背景9:16に合成 → image-to-video。
//   zsh -ic 'cd /Users/veriquest/dev/piyos-adventure/tools && node veo-boss2.mjs'
//   --model=veo-3.1-fast-generate-preview（既定） / --seconds=8
// 出力: _raw/veo_boss2.mp4（コマ確認は contact-sheet、切り出しは別ツール）
// ─────────────────────────────────────────────────────────────────────────────
...
function bossPrompt() {
    return [
        'A GORGEOUS demonic dark CROW boss — deep purple-black plumage wreathed in swirling VIOLET FLAMES, glowing eyes —',
        'performs a menacing aerial animation in SIDE-VIEW, facing LEFT.',
        'Sequence over the clip: (1) hovers in place flapping its huge wings up and down with the violet flame aura trailing,',
        '(2) lunges forward into a steep diving swoop with wings tucked, (3) flares both wings wide unleashing a burst of glowing feathers,',
        '(4) recoils backward as if struck, then (5) settles back into hovering.',
        'It stays CENTERED in the frame and does NOT travel off-screen.',
        'Keep its exact appearance, colors, violet flame aura, proportions and painterly pixel-art style IDENTICAL to the input image.',
        'Plain flat solid bright-green background, no other objects, no camera movement, no zoom, no panning.',
    ].join(' ');
}
...
async function buildSeed() {
    ...
    let basePath = path.join(RAW_DIR, 'boss2_idle_raw.png');
    ...
    const seedPath = path.join(RAW_DIR, 'veo_boss2_seed.png');
...
async function main() {
    ...
    const mp4 = path.join(RAW_DIR, 'veo_boss2.mp4');
