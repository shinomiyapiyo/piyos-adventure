// ─────────────────────────────────────────────────────────────────────────────
// veo-frames-to-idol.mjs — アイドルぴよ: Veoの緑背景フレームを walk スプライトに変換（veo-frames-to-witch.mjs のアイドル版・1.667）。
// 緑クロマキー → デスピル → トリム → skin_idol_idle の身長/足元に合わせて64×64。
//   前段: ffmpeg -i _raw/veo_idol_walk.mp4 _raw/veo_frames_idol/f_%03d.png
//   zsh -ic 'cd tools && node veo-frames-to-idol.mjs --frames=22,30,38,46 --commit'
//   --frames=a,b,c,d  walk_1,2,3,4 に割り当てるフレーム番号 / --bright --sat 色調整 / --commit で images/ 反映
// 出力: _raw/veo_idol_walk_<n>_norm_(64|256).png（--commit で images/skin_idol_walk_<n>.png）
// ─────────────────────────────────────────────────────────────────────────────
...
const OUT = 64;
// ⚠**出力先のスキン名はここだけ**。他のスキンからコピーして作る時は必ずこれだけ直す。
//   1.667: 魔女版から sed でコピーした際に `skin_witch_${key}.png`（テンプレートリテラル）が
//   置換漏れし、**魔女ぴよの歩行スプライトを上書きした**。定数化＋書き出し前チェックで再発を防ぐ。
const SKIN = 'idol';
...
async function main(){
  const idle = await rawRGBA(await fs.readFile(path.join(IMAGES_DIR,`skin_${SKIN}_idle.png`)));
