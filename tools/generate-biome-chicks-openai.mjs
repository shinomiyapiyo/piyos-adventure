// ─────────────────────────────────────────────────────────────────────────────
// 🚫🚫🚫 使用禁止（2026-07-07）🚫🚫🚫
// このツールは「モーション差分コマをOpenAIで1枚ずつ独立生成」する方式で、コマ間で見た目が揺れて
// 動きが崩れる（Ver.1.367で失敗）。モーション（歩行/飛行）の差分コマは必ず Veo 動画から切り出すこと。
// → 歩行: veo-enemy-walk.mjs + veo-frames-to-enemy.mjs / 飛行: veo-enemy-fly.mjs + veo-frames-to-flying.mjs
// 立ち絵（1枚）だけならOpenAI可。ルール: メモリ piyo-sprite-motion-rule。以下は歴史的経緯のため残置。
// ─────────────────────────────────────────────────────────────────────────────
