// ─────────────────────────────────────────────────────────────────────────────
// veo-frames-to-cyber.mjs — サイバーぴよ: Veoの緑背景フレームをスプライトに変換（veo-frames-to-samurai.mjs のサイバー版）。
// 緑クロマキー → デスピル → トリム → skin_cyber_idle の身長/足元に合わせて64×64。
//   前段: ffmpeg -i _raw/veo_cyber_<action>.mp4 _raw/veo_frames_cyber_<action>/f_%03d.png
//   zsh -ic 'cd tools && node veo-frames-to-cyber.mjs --action=walk --keys=walk_1,walk_2,walk_3,walk_4 --frames=22,30,38,46 --commit'
//   zsh -ic 'cd tools && node veo-frames-to-cyber.mjs --action=jumpfall --keys=jump,fall --frames=18,40 --commit'
//   zsh -ic 'cd tools && node veo-frames-to-cyber.mjs --action=dive --keys=dive --frames=52 --commit'
//   --keys / --frames は同数で対応。--bright --sat 色調整 / --scaleup 全身スケール / --commit で images/ 反映
// 出力: _raw/veo_cyber_<key>_norm_(64|256).png（--commit で images/skin_cyber_<key>.png）
// ─────────────────────────────────────────────────────────────────────────────
...
const FRAMES_DIR = path.join(RAW_DIR, `veo_frames_cyber_${ACTION}`);
...
async function main(){
  ...
  const idle = await rawRGBA(await fs.readFile(path.join(IMAGES_DIR,'skin_cyber_idle.png')));
  ...
  for(let k=0;k<KEYS.length;k++){
    ...
    await fs.writeFile(path.join(RAW_DIR,`veo_cyber_${key}_norm_64.png`), norm);
    await fs.writeFile(path.join(RAW_DIR,`veo_cyber_${key}_norm_256.png`), await sharp(norm).resize(256,256,{kernel:'nearest'}).png().toBuffer());
    console.log(`${key} <= f_${FRAMES[k]}  content=${bb.w}x${bb.h} S=${sv.S.toFixed(3)} V=${sv.V.toFixed(3)} -> ${tW}x${tH}`);
    if(COMMIT){ await fs.writeFile(path.join(IMAGES_DIR,`skin_cyber_${key}.png`), norm); console.log(`  ✓ images/skin_cyber_${key}.png`); }
  }
}
