// veo-ninja-walk.mjs — 忍者ぴよの横向き歩行クリップを Veo 3.1 で生成（veo-walk.mjs の忍者版）。
// 種= _raw/ninja_anchor_1_1024.png（確定アンカー）を緑背景9:16へ合成 → image-to-video。
// 実行: zsh -ic 'cd tools && node veo-ninja-walk.mjs'   出力: _raw/veo_ninja_walk.mp4
...
const BASE    = getArg('base') || 'ninja_anchor_1_1024.png';
const SEED_OUT = path.join(RAW_DIR, 'veo_ninja_seed_green.png');
const MP4_OUT  = path.join(RAW_DIR, 'veo_ninja_walk.mp4');
const GREEN = { r: 0, g: 200, b: 0, alpha: 1 };
const W = 720, H = 1280, FOOT_Y = 1185;

const PROMPT = [
  'A 2-heads-tall chibi pixel-art girl in a bright YELLOW NINJA outfit with a yellow hood (her cute face visible),',
  'a small yellow scarf and black gloves and boots, walks in place in a smooth, clear SIDE-VIEW walking cycle, facing right.',
  'Her legs clearly step: one foot forward, then both legs pass close together between steps, then the other foot forward —',
  'a natural open-close-open walk. Her scarf and the small hair tuft from the back of her hood sway slightly with each step.',
...
const NEG = 'walking out of frame, background change, camera motion, panning, zoom, extra characters, text, watermark, blur, realistic 3d render, out of frame';
