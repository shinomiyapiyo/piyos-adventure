// veo-boss2-frames.mjs — veo_boss2 の指定コマを緑クロマキー→128px透過で書き出し、闇のカラスの各ポーズにする。
// 事前に _raw/boss2_frames/f_NNN.png（ffmpegで全コマ抽出）が必要。
//   node veo-boss2-frames.mjs --idle=161 --flap=173 --dive=37 --shoot=117 --damaged=109
//   --preview のみ付けると images/ は上書きせず _raw/boss2_preview.png だけ作る（判定用）
// 出力: images/boss2_<pose>.png（128透過） ＋ _raw/boss2_preview.png（5ポーズ一覧）
...
const OUT = 128;
const POSES = ['idle', 'flap', 'dive', 'shoot', 'damaged'];
const picks = {};
...
async function main() {
    ...
    for (const p of POSES) {
        ...
        if (!PREVIEW_ONLY) { await fs.writeFile(path.join(IMAGES_DIR, `boss2_${p}.png`), sprites[p]); console.log(`  ✓ images/boss2_${p}.png ← f_${picks[p]}`); }
    }
    ...
    await sharp({ create: { width: done.length * CW, height: CW + LABEL_H, channels: 4, background: { r: 24, g: 22, b: 40, alpha: 1 } } })
        .composite(comps).png().toFile(path.join(RAW_DIR, 'boss2_preview.png'));
    console.log(`✓ _raw/boss2_preview.png（${done.length}ポーズ）`);
}
