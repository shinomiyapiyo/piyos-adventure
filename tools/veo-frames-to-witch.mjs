// ─────────────────────────────────────────────────────────────────────────────
// veo-frames-to-witch.mjs — 魔女ぴよ: Veoの緑背景フレームを walk スプライトに変換（veo-frames-to-skin.mjs の魔女版）。
// 緑クロマキー → デスピル → トリム → skin_witch_idle の身長/足元に合わせて64×64。
//   前段: ffmpeg -i _raw/veo_witch_walk.mp4 _raw/veo_frames_witch/f_%03d.png
//   zsh -ic 'cd tools && node veo-frames-to-witch.mjs --frames=22,30,38,46 --commit'
//   --frames=a,b,c,d  walk_1,2,3,4 に割り当てるフレーム番号 / --bright --sat 色調整 / --commit で images/ 反映
// 出力: _raw/veo_witch_walk_<n>_norm_(64|256).png（--commit で images/skin_witch_walk_<n>.png）
// ─────────────────────────────────────────────────────────────────────────────
...
const FRAMES_DIR = path.join(RAW_DIR, 'veo_frames_witch');
...
async function main(){
  const idle = await rawRGBA(await fs.readFile(path.join(IMAGES_DIR,'skin_witch_idle.png')));
  ...
  for(let k=0;k<FRAMES.length;k++){
    ...
    await fs.writeFile(path.join(RAW_DIR,`veo_witch_walk_${k+1}_norm_64.png`), norm);
    await fs.writeFile(path.join(RAW_DIR,`veo_witch_walk_${k+1}_norm_256.png`), await sharp(norm).resize(256,256,{kernel:'nearest'}).png().toBuffer());
    console.log(`${key} <= f_${FRAMES[k]}  content=${bb.w}x${bb.h} S=${sv.S.toFixed(3)} V=${sv.V.toFixed(3)} -> ${tW}x${tH}`);
    if(COMMIT){ await fs.writeFile(path.join(IMAGES_DIR,`skin_witch_${key}.png`), norm); console.log(`  ✓ images/skin_witch_${key}.png`); }
  }
}
