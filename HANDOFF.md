...
> 最終更新: **Ver.1.370**（2026-07-03）。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
...
- **Ver.1.370 をコミット**（デバッグONで🥚50個・要 push）: `handleDebugTap`（core-state.js・10連タップ切替）のON時に `gameSettings.goldenEggs = 50; saveSettings();` を追加（エッグこうかんの動作チェック用）。**OFF時はエッグ保持**（お金scoreと同じ「お店/交換所チェックのため保持」ポリシー）。⚠永続保存に書くため実プレイのエッグ数を上書きする（デバッグは開発者専用機能なので許容・ユーザー指示どおり「50個になる」を literal 実装）。
