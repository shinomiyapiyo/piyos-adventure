...
> 最終更新: **Ver.1.387 まで push済／1.388・1.389 が commit済・push待ち**（2026-07-07）。1.385=フクロウ再設計＋ボスHPスケール統一／1.386=ボス戦の空中雑魚R6以降／1.387=飛行雑魚v2（バイオーム見た目）／1.388=図鑑スクロール修正／1.389=取れないコイン/アイテム/エッグの位置修正。**1.388・1.389はコードのみ＝10分待ち不要**（1.387は画像追加版だったので公開まで10分猶予）。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
>
