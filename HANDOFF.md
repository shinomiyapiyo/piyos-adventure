...
> 最終更新: **Ver.1.384 まで push済／Ver.1.385（フクロウ再設計＋ボスHPスケール統一）が commit済・push待ち**（2026-07-07）。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。※1.385は画像変更なし＝コードのみなので「デプロイ後10分待ち」は不要。
