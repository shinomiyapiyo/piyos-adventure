...
>
> 🔜 **次セッション: 全体レビューの残タスクあり＝冒頭の「🔍 全体レビュー残タスク(P2/P3)」から着手を提案**（Ver.1.403 push済／Ver.1.404=P1対応 ローカルcommit済・push待ち＝ユーザーが`git push`。デバッグ隠しコマンド(ポーズタイトル10連打)はユーザー希望で温存中＝ネイティブ前に要無害化）。直近はユーザーのバッチ指示（1.398-1.403）＋全体レビューP1（1.404）を実装。指示が無ければ P2 → [[piyo-gameplay-backlog]] の順で提案。**参照**: ボス調整ノブ=下の「▶ 調整ノブ早見表」／敵挙動=「地上敵物理まとめ」／スキン効果=「▶ スキン特殊効果」。**運用**: HTML1行でも変えたら版数+0.001(index.html3箇所＋sw.js CACHE_NAME)＋回答末尾に版数／pushはClaudeがcommitまで→ユーザーが`git push`のみ／**Suno音源は使う前にタグ全除去**([[piyo-suno-audio-tags]])／Preview検証はSW全消し＋CACHE_NAME上げてから。
>
