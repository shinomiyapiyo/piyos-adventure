...
> 最終更新: **Ver.1.371**（2026-07-03）。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
...
- **Ver.1.371 をコミット**（うずらの向き修正・要 push）: うずらだけ**逆向きに歩く**とユーザー指摘（1.368で付けた `--flop` が二重反転になっていた＝動画の向きを静止画で見誤った。**教訓再確認: スプライトの向きは必ずゲーム内描画＋目視で判定**）。`--flop` 無しで4コマ再書き出し→ゲーム内で全敵の向きが揃うことをスクショ確認。quailの正しい書き出しコマンド=`node veo-frames-to-enemy.mjs --id=quail --frames=40,43,46,49`（flopなし）。
