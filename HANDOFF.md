...
> 最終更新: **Ver.1.372**（2026-07-03）。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
> 公開URL: https://shinomiyapiyo.github.io/piyos-adventure/

## 現在地サマリ（← 次セッションはまずここ / 2026-07-03）
- **現在 Ver.1.372。全コミット push＆公開済み・未pushなし・作業ツリークリーン・一時ファイルなし。** 公開/CDNとも1.372で、うずら画像も正しい向きを配信中（素URLハッシュ一致で裏取り済み）。
- **唯一の未確認（ユーザー実機タスク）**: ユーザーがアプリを更新して**うずらが進行方向を向いて歩くか**を最終確認するだけ。CDNは既に正しい画像を持つので**今すぐ更新してOK**（10分待ち不要）。※これはネットワーク瞬断で push が一度失敗し、再push→公開まで見届けた直後の引き継ぎ。
- **直近の作業の締め**: 雑魚のバイオーム連動バリエーション（砂漠うずら/雪山シマエナガ/夜ふくろう）を Veo正規フローで作成→モーション脈動解消（1.369）→うずら向き修正（1.371）→CDNキャッシュ起因の再配信（1.372）まで完了。**雑魚バリエーションv1は完了**。
...
- **次の候補**: 雑魚v2=飛行ひよこのバイオーム差し替え／新ボス／ずかん／大型=Capacitorネイティブ化・忍者アバター（課金目玉）。エッグこうかんの品追加（リカラー/リザルトフレーム/練習チケット）も保留中。
