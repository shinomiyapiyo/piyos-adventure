...
> 最終更新: **Ver.1.392 まで push済／1.393 が commit済・push待ち**（2026-07-07）。直近: 1.388=図鑑スクロール修正／1.389=取れないコイン等／1.390-1.391=地上敵に物理(フェーズ1土台+フェーズ2種類別)／1.392=飛行雑魚を図鑑追加＋シマエナガのジャンプ調整／**1.393=①エッグ交換のポーチ選択でスクロールが最上部へ飛ぶ修正(updateTitleShopUIでscrollTop保存/復元)②ポーチ交換時に「きせかえ装備」案内が誤表示→pouch専用メッセージに(tshop_keeper_egg_bought_pouch/owned_pouch)③きぐるみ説明でんきネズミ→きいろネズミ(IP)④ボスのrooster/hawk説明から奇数/偶数ラウンド記述を削除(5体ローテで不正確)⑤図鑑の敵説明に挙動追記(シマエナガ=跳ねる/ゴールデン=高台ジャンプ/ニワトリ=引き返す/草原=穴に落ちる)**。全てコードのみ＝10分待ち不要。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
>
