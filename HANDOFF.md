...
> 最終更新: **Ver.1.393 まで push済／1.394 が commit済・push待ち**（2026-07-07）。直近: 1.390-1.391=地上敵に物理(土台+種類別挙動)／1.392=飛行雑魚を図鑑追加＋シマエナガのジャンプ調整／1.393=エッグ交換UI/図鑑文言/ボス説明の修正5件／**1.394=①シマエナガのジャンプ滞空を伸ばし上下をゆっくりに(`ENEMY_JUMP_V -13→-11`＋ジャンプ中だけ`ENEMY_JUMP_GRAVITY=0.5`＝apex維持で滞空37→43f)②全枠が魔法ポーチ(通常枠0)でも復活薬を「今回かぎり(保存されない)」で購入可に(`isTempReviveCase`・保存不可を説明してはい/いいえ・通常枠へ一時追加＝死亡時の自動復活/手動使用が効く・updateStockUIが一時オーバーフロー枠も描画)**。全てコードのみ＝10分待ち不要。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
>
> 🔜 **次セッション最優先: 復活薬購入(1.394②)の再検討・簡素化**（ユーザー指摘・未着工。1.394自体は動作検証済＝ジャンプ調整も含むので push はOK）。**指摘の要点**: 復活薬は死亡時に自動消費される（index.html:5185付近が `stockState.items` を走査して自動復活）ので**原理的に持ち越せない**。ゆえに1.394の「一時枠/保存できない」機構（`isTempReviveCase` gameplay.js:1351／buyStageItemの一時追加 gameplay.js:906／updateStockUIのオーバーフロー描画 gameplay.js:1495／i18n `shop_keeper_revive_nosave_confirm`）は**過剰かもしれない**。**やること3点**: **(1) 手動使用の是非**＝復活薬は現状タップで手動使用できる（`useStockItem`→`shopItem.stockEffect()`=HP2+バリア gameplay.js:1412／タップ発火 bootstrap.js:390）。ユーザー見解＝「復活薬は死亡時の自動発動のみが正、手動使用は誤り」→**手動使用を無くす方向で要確認・実装**。**(2) リタイヤ持ち越し対策**＝リタイヤ（`confirmRetire` index.html:3013）で復活薬を `stockState.items` から**強制削除**（死なずにリタイヤした時の持ち越しを潰す）。※`stockState.items` は新ラン開始で `=[]` リセットされる（index.html:3410・reset/startGameか要確認）ので実際に持ち越すか含めて確認。**(3) (1)(2)の結論次第で1.394の一時枠機構を簡素化**（手動使用が無く、リタイヤ削除があれば、`temp`旗や「保存できない」説明は簡略化できる可能性）。
>
