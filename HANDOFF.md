...
> 最終更新: **Ver.1.387 まで push済／1.388〜1.390 が commit済・push待ち**（2026-07-07）。1.385=フクロウ再設計＋ボスHPスケール統一／1.386=ボス戦の空中雑魚R6以降／1.387=飛行雑魚v2／1.388=図鑑スクロール修正／1.389=取れないコイン/アイテム/エッグ位置修正／**1.390=地上敵に物理（フェーズ1土台: 重力/地形当たり/穴落下/スポーン地形合わせ/全種stop）。フェーズ2（種類別の個性=ジャンプ/引き返す）は未実装＝次**。全てコードのみ＝10分待ち不要。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
>
> 🚧 **地上敵物理フェーズ2（次にやる・未実装）**: 種類別挙動を`updateEnemyPhysics`(index.html)に追加。**シマエナガ(enaga_walk雪chick)=穴も高台もジャンプ**（穴幅220pxを越すにはジャンプ時に前方リープ=velX一時増を推奨）／ゴールデン=高台ジャンプ／ニワトリ(mama)=壁で引き返す(velX反転)／他chick系=stop据置。**⚠バイオーム雑魚の行動個性化に方針転換**（従来「見た目のみ」→enagaだけ挙動も変える）。飛行敵は現状維持。
>
