...
> 最終更新: **Ver.1.391 まで push済／1.392 が commit済・push待ち**（2026-07-07）。直近: 1.387=飛行雑魚v2／1.388=図鑑スクロール修正／1.389=取れないコイン/アイテム/エッグ位置修正／1.390=地上敵に物理（フェーズ1土台）／1.391=地上敵の種類別挙動（フェーズ2）／**1.392=①飛行雑魚(ハゲタカ/白フクロウ/コウモリ)を図鑑に独立エントリ追加(敵7→10・全39→42)②シマエナガのジャンプ調整（`ENEMY_JUMP_V -16→-13`=8割・`ENEMY_LEAP_VX 7→4.5`=避けやすく。広い穴は落ちる=許容）**。全てコードのみ＝10分待ち不要。リポジトリ: `piyos-adventure`（GitHub Pages, **Actions方式で自動公開**）。
>
