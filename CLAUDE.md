...
## 🚨 絶対ルール — 生成した画像は必ずそのターンで出す（2026-08-01 ユーザー厳命）

> **画像を生成したら、生成したそのターンの中で必ずユーザーに表示する。例外なし。**

- 文章で「◯◯ができました」「候補3枚できました」と説明するだけで**画像を出さないのは違反**。
  ユーザーは画像を見られないので**判断できない**（実際にやってしまい指摘された）
- **「見せてほしい」と言わせるのも違反。** 別ターンで出し直すのは無駄な手間をかけさせている
- 手段は `SendUserFile`（`display: "render"`）。**候補は全部出す**。比較用に作った検品画像も一緒に出す
- ⚠**検品（実測）は出す前に済ませる**が、検品を理由に**表示を次のターンに遅らせてはいけない**。
...
- **ストア版数の上げ幅は固定**（2026-07-31 ユーザー指示・確認不要で機械的に適用する）:
  - **明らかに新しい機能が増えた版 → 0.1 上げ**（例 1.6 → 1.7）
  - **それ以外（不具合修正・既存仕様の調整・バランス変更・UIの整理・演出の撤去など）→ 0.0.1 上げ**（例 1.6 → 1.6.1）
  - 「新しい機能」＝新ステージ／新キャラ／新モード／新画面のように**遊べるものが増えた**こと。
    既存アイテムの入手経路や価格の調整、表示の追加は**機能追加に数えない**
  - ⚠**この判定はビルド前（`RELEASE_PLAYBOOK.md` §1 の版数上げ）に済ませる**。版数文字列は AAB と
    xcarchive に焼き込まれるため、後から変えると**両方の作り直し**になる（1.7→1.6.1 で実際に起きた）
  - 内部番号（iOS `CURRENT_PROJECT_VERSION` / Android `versionCode`）は上げ幅に関係なく**毎回 +1**
- **リリースの手順は `RELEASE_PLAYBOOK.md` が正**（2026-07-30 ユーザー指示で標準化）。版数上げ→ビルド→成果物の検証→
  **Claude がブラウザを操作して Play Console と App Store Connect に提出するところまで**が毎回の流れ。
  ⚠ユーザーにしか出来ないのは「ストアへのログイン」「AAB とスクリーンショットのファイル選択」「iOSビルドのアップロード」だけ
  （ブラウザ自動化はネイティブのファイルダイアログを操作できない）。版ごとの実測値は `RELEASE-<版>.md` に残す
- ネイティブ（Capacitor）: iOS=App Store公開済み／Android=Playクローズドテスト中（⚠製品版には出さない・組織アカウント待ち）
