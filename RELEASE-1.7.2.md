...
**両ストアとも提出完了。** ⚠実機テストプレイ未了のまま提出した（ユーザーに提示のうえ続行）。

### Play（クローズドテスト Alpha）
- AAB `14 (1.7.2)` をアップロード → リリース名は自動で `14 (1.7.2)`
- リリースノートは1つの textarea に `<ja-JP>`/`<en-US>` 併記（ja 206字・en 373字）→ **「リリースノート: 2 言語」**を確認
- レビュー画面の実測: **新規インストール 62.3MB（−696KB）／アップデート 4.11MB**。
  サポート対象デバイスの増減は全フォームファクタ 0
- 警告は1件のみ＝**難読化解除ファイルが無い**（`minifyEnabled false` なので該当なし・無視でよい）
- 段階的な公開の割合は**最初から 100.0 が入っている**（触らなくてよい）
- 保存 → 「概要に移動」→ **「1 件の変更を審査に送信」→「変更を審査に送信」** →
  見出しが**「審査中の変更」**に変化＝完了（クイックチェックは残り約14分の途中でも送信できた）

### ASC（iOS 1.7.2 / build 15）
- サイドバーの「+」→ **新規バージョン 1.7.2** を作成 → ビルド15を紐づけ → 保存
- 「含まれるアセット ▸ アプリアイコン」が**新アイコンで表示される**＝差し替えの最終確認になる
- 新機能 ja 206字 / en 373字、プロモ文 ja 80字 / en 131字（170の境界を避けた）を**言語ごとに別保存**。
  切り替えて**日本語が消えていないことも確認**
- 審査メモは前版（1.7.1）の 1,541字が残っていたので **1,799字に全面書き直し**
- リリース方法は前版のまま（自動でリリース／すべてのユーザ向けに今すぐ／既存の評価を維持）
- 「審査用に追加」→ 提出物の下書きに `iOSアプリ1.7.2 / 1.7.2 (15)` → **「審査へ提出」→「1項目が提出されました」**
  → サイドバーが**「審査待ち」**に変化＝完了

### ⚠今回わかった操作上の罠（次回そのまま効く）

- **内蔵ブラウザの `resize_window` で実寸より広げるとクリックが一切効かなくなる。**
  1600×1000 に広げた状態ではボタンの座標が合っていても無反応（ref 指定でも同じ）。
  **`resize_window` の `preset: "desktop"`（実寸＝864幅）に戻すと直る**。1.7.1 の
  「ASCの『+』は1600pxに広げないと出ない」は、**実寸だとサイドバーがハンバーガーに畳まれるだけ**で、
  **タイトル横の「iOSアプリ バージョン X ⌄」を押せば同じメニューが開き「+」も出る**＝広げる必要はない
- クリック座標は**スクリーンショットのピクセル座標**で渡す（CSS座標ではない）
- Play の「新しいリリースを作成」など一部のボタンは押しても反応しないことがある。
  **一度リロードしてレイアウトが正常に描き直されると通る**
- ASC の「保存」は**1回目のクリックが空振りすることがある**。`保存`ボタンが disabled になるまで押す
