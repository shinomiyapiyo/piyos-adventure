...
function ugMakeEnemy(pe) {
    ...
    // ⚠1.612: ここは元々 `1 + (gameRound-1)*0.3`（地上のラウンド倍率）だったが、**ラウンドで変えない**
    //   ことにした（ユーザー決定「地底は速さの概念を取っ払っている。ボスのみ例外として速くなる」）。
    //   青天井だったため R28 で一番遅いニワトリまでプレイヤーより速くなっていた。詳細は
    //   core-state.js の UG_ENEMY_SPEED_MULT のコメント。⚠ボスは ugMakeEnemy を通らないので影響なし。
    s *= UG_ENEMY_SPEED_MULT;
    // ⚠地底は**プレイヤーも UG_SPEED_RATE 倍**で歩いている（1.544）。敵にだけ地上の値を使うと
    //   敵 2.2〜3.1px/f 対 プレイヤー 3.0px/f ＝ ほぼ同速になり、避けようがなくなる。
...
function ugCollapseSkully(e) {
    ...
    // 図鑑もスコアも**その個体の初回の崩壊だけ**（✅ユーザー決定1.676）。再生するので、
    // 同じ骨を踏み続けてコンボ・必殺ゲージ・スコア・図鑑の撃破数を無限に稼げてしまうのを防ぐ。
    // ⚠1.675まで zukanAddKill だけがこのガードの外に出ていた＝**図鑑の撃破数だけ毎回加算**されていた
    //   （スコアは初回のみ、という取りこぼし。ユーザー指摘）。図鑑の扱いを他の敵と揃える＝
    //   「1体につき撃破数1」。発見(seen)は zukanAddKill の中で立つので初回の崩壊で必ず登録される。
    // ⚠倒せない敵なので撃破イベントが存在しないが、崩壊もプレイヤーの能動的な行動が要る点は撃破と同じ
    //   ＝1.474の「倒してないのに図鑑に載る」問題には当たらない（1.563の判断を維持）。
    if (!e.scored) {
        e.scored = true;
        zukanAddKill('enemy:skully');
        registerKill(UG_SKULLY_SCORE, e.x + e.width / 2, e.y);
    }
}
...
//   **同じ日次枠を共有**させる。
// ⚠見ているのは「今日もう**抽選した**か」＝canDrawDailyEggToday()（「貰ったか」ではない・1.685で表現を修正）。
//   よって 2,500mの30%を外し、救済の4,300mに届く前に力尽きた日は、**1個も貰っていなくても**その後のランで
//   巫女を倒しても報酬エッグは出ない（＝1日1抽選の据え置き）。「貰えていなければ巫女で救済する」に変えたい場合は、
//   ここと index.html の抽選ブロックを lastGoldenEggDate（＝取得した日）基準に揃える必要がある＝仕様変更。
