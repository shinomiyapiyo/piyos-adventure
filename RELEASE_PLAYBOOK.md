...
> ✅**2026-09-04: この手順はスキル化した → `native-store-release`**
> （実体 `.claude/skills/native-store-release/`・`~/.claude/skills/` からリンク済みで**他のアプリでも使える**）。
> 作業時はスキル側を開くこと。成果物の検証は `scripts/verify-release-artifacts.sh` が自動でやる。
> このファイルは**ぴよ氏の冒険に固有の値**（パス・Team ID・スクショの置き場・実測値）の控えとして残す。
...
3. **言語ごとに入れて、言語ごとに保存する**（⚠ja/en は別保存。片方だけ保存して切り替えると消える）
   ...
   入力だけは止めずに済ませる。**ボタンが押せない件は `.click()` で黙って迂回せずユーザーに伝える**
   （2026-08-12 指示・下記の罠の表を参照）。
...
| ブラウザペインが隠れてクリックが効かない | `computer` がタイムアウト＋スクショが白紙＋`document.visibilityState==='hidden'`。**ユーザーに「ブラウザの幅を広げてください」と伝える**（ユーザー側の操作で直る）。⚠`.click()` で黙って迂回しない。**入力（setter）だけは通るので済ませてよい** |
