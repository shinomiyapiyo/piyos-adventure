...
| ✅1.696で修正 | [medium] 地底エンディングの一枚絵にぴよフラッシュUIが残り発動できる |
| ✅1.696で修正 | [low] 11件すべて（極楽まんじゅうの表示／羽根弾が地面下／スキップボタン／割り込みポーズ／トースト2件／旧セーブ移行／ボスのポーズ1F／検証ページ／ミッションの戻る／チュートリアルのきせかえ） |
| ✅仕様（指摘は却下） | [UNCERTAIN] 地底モードの金で永続アップグレードが買える件 → **ラン中に買える永続アイテムは購入時に確定するのが正しい**（✅ユーザー確認・2026-07-30）。今後この種の品が増えても同じ扱い。なお実測でモードの所持金上限は ≒137,400円＝加護の200,000円には届かない |

⚠**1.695の「地底モードは永続化しない」は1.696で差し戻した**（ユーザー仕様: 老婆の劇薬をポーチで持ち帰るのは意図した設計。
ただし**モードをクリアした時だけ**残る＝力尽きたら残らない。転売は売値500円固定＋老婆の買取拒否で塞いである）。
