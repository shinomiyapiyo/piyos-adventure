...
// 距離加算の速さ（1.542調整）。⚠地底は「自分の足で進む＝速いほど距離が増える」ので上限を設けないと、
//   はやあし(1.3倍)所持で 46.8m/秒＝**地上では絶対に出せない速さ**で距離が増えてしまう（地上はLv17以降 36.0m/秒で頭打ち）。
//   そこでカメラ前進を「地上の上限×UG_DIST_RATE」で頭打ちにする。1.0=地上と同じ36m/秒。
//   下げると総加算量(1,230m)は変えずに“増え方”だけ緩やかになる（0.5なら18m/秒・踏破68秒）。
const UG_DIST_RATE     = 1.0;
