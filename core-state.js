...
// ⚠⚠テスト専用スイッチ2（Ver.1.751・「広告のあと音が戻らない」の原因を実機で見るため）⚠⚠
//   true にすると、広告が閉じた直後に**音声の状態**を画面上部のトーストに1回だけ出す。
...
//   'r35'    … **R35開始の瞬間（105,600m）＝5周目の地底「灼熱の階梯」（1.734の新レイアウト）から**（1.734で追加）。
//              ⚠看板の「渡り」は6番目の部屋（列430〜519）＝地底に入ってから約13,760px 先。
//              ⚠高度の並びが 低→高→中→高→低 なので、縦の部屋を5つ通る（落下1・登り2・降り2）。
//   'r6shop' … R6のおみせ手前（11,450m）から。おみせ → 闇のカカシ → 撃破 → R7 → 土管、と通しで見たい時
// ⚠1.624〜1.627にあった 'speedcap'（敵速度の上限を体感するモード）は **1.628で撤廃**。
//   上限が本採用（ENEMY_SPEED_MULT_CAP / _FLY）になり、テスト用の頭打ちを別に持つ意味が無くなったため。
var TEST_START_MODE = 'r35';   // ⚠1.734: 新レイアウトの実機確認のため 'r35' に（TEST_START_AFTER_R6=false の間は無効）
...
// ─── 地上の雑魚・飛行敵の速度倍率（共通） ───
// ⚠**同じ式が spawnEnemy と spawnFlyingEnemy の2箇所に重複していた**のを1つにまとめた（1.624）。
//   将来この倍率に本採用の上限を入れる時も、触るのはここ1箇所だけで済む。
...
// ─── 速度の上限（1.628・実機テストで確定／⚠**ここから先は青天井にしない**） ───
// 上限を入れた理由（1.624〜1.627の実機テストと計測で確定）:
//   ①**踏めなくなる**: 敵が自機のX重なり帯(78px)を通過するフレーム数が5Fを切ると狙って踏めない。
//     上限なしのR33では2.4F・R55では1.6F＝撃破数が運になる。
//   ②**当たりが運になる**: 被弾帯(66px)の通過が2F前後になると、当たる/当たらないが位相まかせになる。
//     さらに接近66px/fを超えると**1フレームで飛び越えて当たらなくなる**（R78相当）。
//   ③**敵のAIが設計を外れる**: 穴/壁の先読みは60px（跳ぶ敵）/10px（ニワトリ）固定、
//     跳躍中の横速度は ENEMY_LEAP_VX=4.5 固定。歩行がこれを大きく超えると「跳んだ瞬間に急減速」に見え、
//     穴の回避も間に合わなくなる。
//   ④実データ（ランキング距離上位25本）では中央値が9,617m＝R5。上限が効くのは**上位2本だけ**＝
//     大多数のプレイヤーの体験は一切変わらない。
// ⚠難度は速さの代わりに「アカバネ(R11〜)・敵の数・構成」で上げる方針（ボスHPを R13 で頭打ちにして
//   攻撃パターンで上げているのと同じ考え方）。
// ⚠1.715でユーザー指示により 7.0 → 8.2 に引き上げた（「敵の速度の上限をもう少しだけ上げたい」）。
//   引き上げ幅の根拠＝**上の①「踏み窓5F」を測り直した実測**（スクロールは Lv21=6,000m以降ずっと 6.0px/f 固定、
//   踏み帯78px、地上雑魚の素の速度の加重平均 1.4375px/f ＝ ひよこ1.0-2.0×2 / 金1.0-2.5 / ママ0.5-1.5）:
//     倍率7.0（現行・R24到達）… 平均の接近16.1px/f → 踏み窓 4.86F
//     倍率8.2（採用・R29到達）… 平均の接近17.8px/f → 踏み窓 4.39F ／ 最速の金ひよこ 2.94F
//     倍率9.4（R33到達）      … 踏み窓 4.00F ＝ ここまで来ると①の限界。**これ以上は上げない**
//   ＝踏み窓の目減りは 0.47F（約10%）に収め、頭打ちを **R24 → R29** へ3ラウンド分伸ばした。
// ⚠**飛行の上限は上げない**（1.715でも据え置き・ユーザー了承）。飛行は素の速度が地上の約2倍(2.0〜4.0px/f)あり、
//   1.626の実測で7.0倍だと**接近0.30秒＝人間の単純反応(約0.25秒)とほぼ同じ**で避けようがなかった。
//   アカバネも飛行なので、**飛行側は速度ではなく頻度で難度を上げる**（下記 diveBirdRate）。
const ENEMY_SPEED_MULT_CAP     = 8.2;  // 地上: 地上通算25ラウンド目＝R29(63,600m)で到達。金ひよこ最速20.5px/f
const ENEMY_SPEED_MULT_CAP_FLY = 4.0;  // 飛行: 地上通算11ラウンド目＝R12(24,000m)で到達。最速16.0px/f

function enemySpeedMult() {
    return Math.min(roundSpeedMultRaw(gameRound), ENEMY_SPEED_MULT_CAP);
}

// 飛行敵は**地上と別の上限**（1.626・ユーザー実機報告「空中の雑魚敵が速すぎて反応できない」）。
// ⚠原因は素の速度差＝同じ倍率を掛けると差がそのまま開く: 地上 base 0.5〜2.5px/f に対し
//   **飛行 base 2.0〜4.0px/f**。7.0倍時の実測 → 地上最速17.5px/f（画面に現れてから触れるまで0.44秒）に対し
//   飛行最速 **28.0px/f（0.30秒）**＝人間の単純反応(約0.25秒)とほぼ同じで避けようがなかった。
// 4.0倍なら飛行は 8.0〜16.0px/f・画面接近22.0・**0.47秒**＝地上の最速(0.44秒)よりわずかに緩い。
// 空の敵は「跳んだ先に居る」形で当たるので、地上と同値(4.35倍)ではなく少し余裕を持たせてある。
function flyingSpeedMult() {
    return Math.min(enemySpeedMult(), ENEMY_SPEED_MULT_CAP_FLY);
}
...
const DIVE_BIRD_RATE      = 0.30; // 出現開始時の割合（空中雑魚のうちこの割合が急降下型になる）
// ⚠1.715: **固定 0.30 をやめ、ラウンドで緩やかに増やす**（ユーザー指示「アカバネも調整して確率を増やすなどしたい」）。
//   直した動機＝速度に上限を入れた 1.628 のコメントは「難度は速さの代わりに**アカバネ・敵の数・構成**で上げる方針」と
//   書いてあるのに、**実際には率も湧き間隔も種類の重みも全部固定**で、R11以降まったく難しくなっていなかった
//   （ユーザー実機報告「R24でしおり／ゲームオーバーになる気配がない」→ 実装を確認して発覚）。
// 設計（1.715・実測から計算してユーザーが選択）:
//   ・飛行雑魚の湧きは 180F待ち＋毎F15%抽選 ＝ **約187F(3.1秒)に1体**で固定（ここは触らない）
//   ・率は R11=30% から**地上ラウンドごとに等間隔で上げ、R38 で 60% に到達**して頭打ち
//     → アカバネの間隔: R11=10.4秒 → R20=7.7秒 → **R24=7.0秒** → R29=6.3秒 → R38=5.7秒
//   ・⚠**60%を上限にする理由**: 飛行の湧き自体は増やさないので、率を上げるほど「ただ飛ぶだけの雑魚」が
//     消えて全部アカバネになる。100%にすると空中の敵が全部攻撃してくる別ゲームになるため、
//     **半分強を残す 60% が上限**。それでも足りなければ次は**飛行の湧き間隔(180F)を詰める**（＝「数」で上げる）
//   ・⚠**到達を R38 まで引っぱるのが要点**（ユーザー判断。当初案の R21 到達は「早すぎる」で却下）。
//     速度の頭打ちが R29 なので、**速度が止まった後もアカバネだけは増え続ける**＝難度の伸びが途切れない
const DIVE_BIRD_RATE_MAX   = 0.60;  // 頭打ちの割合
const DIVE_BIRD_RATE_FULL_R = 38;   // この地上ラウンドで DIVE_BIRD_RATE_MAX に到達
// 実際の割合。⚠段数は **groundRoundIndex（実際に走った地上ラウンド通算）** で数える＝地底ラウンドで
//   段が飛ばないようにする（速度倍率 roundSpeedMultRaw と同じ考え方・1.627で踏んだ罠）。
function diveBirdRate(r) {
    if (r === undefined) r = gameRound;
    var steps = groundRoundIndex(DIVE_BIRD_RATE_FULL_R) - groundRoundIndex(DIVE_BIRD_ROUND);
    var per = (DIVE_BIRD_RATE_MAX - DIVE_BIRD_RATE) / steps;   // 1段あたり ≒ +1.30%
    var n = Math.max(0, groundRoundIndex(r) - groundRoundIndex(DIVE_BIRD_ROUND));
    return Math.min(DIVE_BIRD_RATE_MAX, DIVE_BIRD_RATE + n * per);
}
...
// ═══ 上位ランカー向けの難度調整（1.741・ユーザー指示 2026-08-25）═══
// ⚠**ここまで到達する人はほぼ永久に生き残れてしまう**ので、序盤の救済を段階的に外していく。
//   ⚠**理不尽にはしない**＝跳べない幅や避けられない配置にはしない。削るのは「余裕」だけ。
const WIDE_HOLE_ROUND = 36;   // この周回から高速域の穴の幅を 70〜115px → 90〜160px に広げる
// この周回から雑魚の湧く間隔を詰める（＝画面に同時に居る数が増える）。
// ⚠**敵の速度は上限(地上7.0倍/飛行4.0倍)に張り付いている**ので、ここから先の難度は"数"でしか上げられない。
//   ⚠速度と二重に効かないぶん、増やしすぎると一気に理不尽になる＝**+3割程度に留める**。
const DENSE_ENEMY_ROUND   = 43;
const ENEMY_COOL_BASE     = 90,  ENEMY_PROB_BASE  = 0.4;    // 通常: 平均 92.5フレームに1体
const ENEMY_COOL_DENSE    = 70,  ENEMY_PROB_DENSE = 0.45;   // R43〜: 平均 72.2フレームに1体（約+28%）
const FLY_COOL_BASE       = 180, FLY_PROB_BASE    = 0.15;   // 通常: 平均 182.5フレームに1体
const FLY_COOL_DENSE      = 150, FLY_PROB_DENSE   = 0.18;   // R43〜: 平均 155.6フレームに1体（約+17%）

// ═══ 地上の縦カメラ（1.741・ユーザー指示）═══
// ⚠**滞空が無敵ゾーンになっていた**のを潰すための追従。画面上端より高い所に居る間は
//   敵も弾も画面に入らず、事実上どこからも当たらない時間が作れてしまっていた。
// ⚠**上にしか動かさない**（下は従来どおり固定＝地面より下は見せない）。
// ⚠**復活（上から無敵で降ってくる）では追わない**＝あの演出は変えない。
const SURF_CAM_UP_MAX    = GAME_HEIGHT / 2;   // 上へ追う上限＝半画面(225px)
...
// 地上を一切挟まず、地底の5レイアウトを R1→R5 として連続で遊ぶ。**中身は通常プレイと完全に同一条件**。
...
//   ⚠**ここを「1,2,3,4,5」にしてはいけない**（レイアウトも巫女も1周目のものになってしまう）。
// ⚠1.735: **5面に増やした**（R35「灼熱の階梯」の追加に合わせて・ユーザー指示）。
//   ステージ数の表示（HUD「1/5」）・最終面の判定・リザルトの {total} はすべて
//   この配列の長さから引いているので、ここに1行足すだけで全部そろう。
const UG_MODE_ROUNDS      = [BOSS_CYCLE_ROUNDS, BOSS_CYCLE_ROUNDS * 2, BOSS_CYCLE_ROUNDS * 3,
                             BOSS_CYCLE_ROUNDS * 4, BOSS_CYCLE_ROUNDS * 5];
const UG_MODE_STAGES      = UG_MODE_ROUNDS.length;   // 5
...
//   （獲得バッジの告知は1.638で全バッジ共通の runEarnedBadges に一本化＝ここでは持たない）
var undergroundMode = { active: false, stage: 0, cleared: false };
...
// フィールドの日次エッグを1日に何回まで抽選するか（1.693・✅ユーザー決定「案C」・調整ノブ）。
// ⚠**券を使うのは「抽選した時」**という原則は変えていない（1.455〜）。変えたのは券の枚数（1→2）。
//   理由: 2,500mの30%を外し、7,300mの救済にも届かない日は**1個も貰えないまま その日が終わる**（浅いランなら7割）。
//   1枚だと運の振れが大きすぎるので2枚にした（0個で終わる日: 浅いラン 70%→49%／7,300m到達 35%→12%）。
// ⚠**上限は1日1個のまま**（`lastGoldenEggFieldDate`＝その日フィールドで手に入れた日付 で打ち切る）。
//   「2回引ける」＝2個貰える、ではない。ここを取り違えると供給が倍になる。
const EGG_FIELD_DRAWS_PER_DAY = 2;
...
var gameState = {
    ...
    // ⚠1.719: ぴよフラッシュのゲージを距離で溜める時の基準点（監査で発見）。
    //   定義にも resetGame にも無く、しおり再開で距離が前へ飛ぶと**差分がそのままゲージに化けていた**
    //   （900m地点で中断→再起動→つづきから で 900×0.08=72ポイント／満タン100 がいきなり入る）。
    gaugeDistMark: 0,
...
// 追いつきカメラの強さ（1.643・調整ノブ・ユーザー実機報告「急降下斬りでカメラの移動が追いつかない／
// 連続で撃つとプレイヤーが一番右までいってしまう」）。カメラの前進上限は歩行基準の 3.0px/f 固定だったが、
// 急降下斬りは地底で 14px/f 進むため、1回斬るたびに 11px/f ずつ引き離されて数回で右端に貼り付いていた。
// **本来いてほしい位置(UG_CAM_LEAD)より前に出ているぶんに比例して上限を上げる**＝離されるほど強く追う。
// ⚠1.0（完全同期）にはしない。斬りで前に出る手応えを残すのがユーザー指定。
const UG_CAM_CATCHUP   = 0.09;
...
// ⚠1.622: 「余韻が長い」というユーザー指摘で 300F(5秒) → 180F(3秒) に短縮。撃破から一枚絵までは
//   UG_END_CALM(300F) + これ ＝ 600F(10秒) → 480F(8秒)。拾い時間もそのぶん3秒に縮む。
const UG_END_GRACE      = 180;  // 静まりきってから一枚絵に入るまでの猶予（3秒@60fps）
...
function ug28BuildDivide() {
    ...
    // 合流の大広間（列150-183）で上ルートが降りてくる棚。
    // ⚠1.620: **1段6行(192px)に統一**した。`descend` のカメラは足元の250px下まで映すので、
    //   どの段に立っても**次の段が必ず画面に入る**（実機報告「この位置に来たら下方向が見えなければならない」）。
    //   最後の段(行46)から床(行53)も224pxなので見える。⚠階段は左寄せ・滝は右端(列176-177)へ分けてある。
    var MERGE = [[16, 150, 152], [22, 154, 156], [28, 158, 160], [34, 162, 164], [40, 166, 168], [46, 170, 172]];
    ...
    for (c = 0; c < W; c++) {
        ...
        if (c >= 150 && c <= 183) { carve(2, 52, c); continue; }    // 合流の大広間（棚は後で戻す）
        if (c >= 184) { carve(43, 52, c); continue; }               // 合流後の低層の通路
    ...
    var deco = [
        ...
        [15, 151, 'o'], [21, 155, 'o'], [27, 159, 'o'], [33, 163, 'o'], [39, 167, 'o'], [45, 171, 'o'],
        [52, 155, 'm'], [52, 162, 'v'], [52, 168, 'S'], [52, 181, 'm'], [52, 186, 'v'], [52, 194, 'm'],
        [52, 190, 'F'], [52, 197, '^'], [52, 198, '^'], [49, 188, 'o'], [49, 196, 'o']
    ];
    ...
    // ⚠**滝の列には他のものを一切置かないこと**。FALLS は空きマスにしか入らないので、1マスでも
    //   敵やギミックが被ると**その行だけ滝が抜け、左右で高さの違う2本に割れる**（＝着水が2段になる）。
    //   1.621で実際に起きた（列176の行52に敵を置いていた・ユーザー実機報告「滝が1か所落ちる位置が高い」）。
    var FALLS = [[22, 31, 20, 21], [2, 52, 176, 177]];
...
// ⚠1.620: **合流の大広間だけを別部屋に切り出す**。カメラの `descend`（足元を画面の上寄りに置いて
//   250px下まで映す）は**部屋単位**の設定なので、広間だけを別部屋にしないと下が見えない。
//   ⚠切れ目は列146＝上ルートのトンネルがまだ続いているうちに切り替わる。棚の先端(列149)で
//   切り替えたのでは「降りる直前」に間に合わず、結局最初の段が見えない。
var _UG28_DIVIDE_FULL  = ug28BuildDivide();
var UG28_ROOM_DIVIDE   = _UG28_DIVIDE_FULL.map(function (r) { return r.slice(0, 146); });
var UG28_ROOM_CONVERGE = _UG28_DIVIDE_FULL.map(function (r) { return r.slice(146); });
...
var UG28_LEVEL_ROOMS = [
    ...
    { key: 'divide',   wT: 146, topY: -516, map: UG28_ROOM_DIVIDE },   // ★中508 →（上下分岐）→ 2本が並走
    // ⚠descend:true ＝ここだけカメラが下を向く。上ルートが階段で降りてくるので、
    //   足元の96px下までしか映さない通常の framing では次の段が見えない（1.620・実機報告）。
    { key: 'converge', wT: 54,  topY: -516, map: UG28_ROOM_CONVERGE, descend: true },  // 合流の大広間→低1180
...
// ═══════════════════════════════════════════════════════════════════════════════
// R35（5周目）「灼熱の階梯」 — 1.734／**3分岐に作り直し 1.735**
// ⚠**骨格を既存4種のどれとも変える**（R14で「似すぎ」と言われた教訓）。この面の軸は3つ:
//   ①**上・中・下の3分岐**（ユーザー指示・1.735）。R21は2ルート／R28も2ルートなので、
//     3本に分かれるのはこの面が初めて。**3本それぞれの性格を変える**のが肝:
//       上「梁の道」… 穴も溶岩も無い代わりに**バーと飛行敵が最も濃い**（落ちないが削られる）
//       中「渡り」  … **溶岩の海を動く床で渡る**。落ちれば復帰＝最も事故率が高い代わりに実入りが多い
//       下「窯の底」… 平地だが**敵とトゲが最も多い**。噴出口もここだけ多い
//   ②高度の並びは **低→中→（高/中/低の3並走）→低→高→低**。
//   ③部屋数13（最多）。
// ⚠**3本は岩盤で完全に仕切る**（R28で確立した約束）。上ルートの床に穴を開けない・中ルートの
//   溶岩の下は岩を残す＝**途中でルートを変えられない**。分かれ目は分水嶺（列40〜63）だけ。
// ⚠密度は R28（バー42/火の玉14/敵97/トゲ31/動く床8）比で大きく上げる。⚠ただし**避けにくさでは
//   上げない**（周期・予告・当たり判定は1周目のまま）＝SPEC §5.4 の方針。
// ⚠合計 24+60+32+40+176+54+48+32+72+32+44+136+40 = **790タイル**（他の4種と同じ＝距離加算は不変）。
//   店は参道の列29＝**643タイル目**／巨像は列121＝**735タイル目**（4種すべてと同じ絶対位置）。
// ═══════════════════════════════════════════════════════════════════════════════

// ── 部屋1「奈落」 縦(降) 24タイル×57行【天井 → 低1180】────────────────
// ⚠**歴代で最も深い導入の落下**（1,728px＝R28の1,024pxの1.7倍）。落ちること自体が演出なので
//   ギミックは置かない。⚠落下線（列0〜4）には何も置かない＝置くと避けようが無い被弾になる。
var UG35_ROOM_PLUNGE = [].concat(
    ugRows(2, 24, '#', [[1, '   ']]),                                       // 0-1  天井の裂け目（列1-3）
    [ugRow(24, ' ', [[0, '#'], [23, '#']])],                                // 2
    [ugRow(24, ' ', [[0, '#'], [1, 'o'], [16, 'BBBB'], [23, '#']])],        // 3
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 4-5
    [ugRow(24, ' ', [[0, '#'], [6, 'BBB'], [23, '#']])],                    // 6
    ...
    [ugRow(24, ' ', [[0, '#'], [1, 'o'], [23, '#']])],                      // 11
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 12-13
    [ugRow(24, ' ', [[0, '#'], [6, 'BB'], [23, '#']])],                     // 14
    [ugRow(24, ' ', [[0, '#'], [2, 'o'], [23, '#']])],                      // 15
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 16-17
    [ugRow(24, ' ', [[0, '#'], [18, 'BBB'], [23, '#']])],                   // 18
    [ugRow(24, ' ', [[0, '#'], [1, 'o'], [23, '#']])],                      // 19
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 20-21
    [ugRow(24, ' ', [[0, '#'], [8, 'BBB'], [23, '#']])],                    // 22
    [ugRow(24, ' ', [[0, '#'], [2, 'o'], [23, '#']])],                      // 23
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 24-25
    [ugRow(24, ' ', [[0, '#'], [15, 'BBBB'], [23, '#']])],                  // 26
    [ugRow(24, ' ', [[0, '#'], [1, 'o'], [23, '#']])],                      // 27
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 28-29
    [ugRow(24, ' ', [[0, '#'], [6, 'BB'], [23, '#']])],                     // 30
    [ugRow(24, ' ', [[0, '#'], [2, 'o'], [23, '#']])],                      // 31
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 32-33
    [ugRow(24, ' ', [[0, '#'], [16, 'BBB'], [23, '#']])],                   // 34
    [ugRow(24, ' ', [[0, '#'], [1, 'o'], [23, '#']])],                      // 35
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 36-37
    [ugRow(24, ' ', [[0, '#'], [9, 'BBB'], [23, '#']])],                    // 38
    [ugRow(24, ' ', [[0, '#'], [2, 'o'], [23, '#']])],                      // 39
    ugRows(2, 24, ' ', [[0, '#'], [23, '#']]),                              // 40-41
    [ugRow(24, ' ', [[0, '#'], [17, 'BBB'], [23, '#']])],                   // 42
    [ugRow(24, ' ', [[0, '#'], [23, '#']])],                                // 43
    // ⚠行44-53は**右壁を開ける**（＝燠火の原の空間 860〜1148 と同じ高さ）
    ugRows(10, 24, ' ', [[0, '#']]),                                        // 44-53
    ugRows(3, 24, '#', [[15, 'LL']])                                        // 54-56 床 1180（列15-16は溶岩）
);
// ── 部屋2「燠火（おきび）の原」 横 60タイル×15行【低層 1180】────────────
// 着地してすぐの部屋。⚠**列6までは無風**にする（落下直後の事故死を防ぐ・R7の教訓）。
var UG35_ROOM_EMBERFIELD = [].concat(
    ugRows(2, 60, '#'),                                                     // 0-1 天井
    ugRows(3, 60, ' '),                                                     // 2-4
    [ugRow(60, ' ', [[16, 'H'], [44, 'H']])],                               // 5 天井バー
    ugRows(2, 60, ' '),                                                     // 6-7
    [ugRow(60, ' ', [[10, 'o'], [26, 'o'], [34, '1'], [42, 'o']])],         // 8
    [ugRow(60, ' ', [[20, '====='], [46, '=====']])],                       // 9 逃げ場
    [ugRow(60, ' ', [[14, 'o'], [30, '4'], [50, 'o']])],                    // 10
    // 11 床のギミック＋敵。⚠動く床は**溶岩の左を1列あけて**置く（跳び移る間を作る）
    [ugRow(60, ' ', [[6, 'm'], [10, '^^'], [13, 'F'], [18, 'e'], [22, 'v'], [24, '^^'],
                     [28, 'G'], [31, 'S'], [35, 'MMMM'], [41, 'v'], [44, '^^'],
                     [47, 'F'], [50, 'm'], [53, 'e'], [56, 'S'], [58, 'v']])],
    [ugRow(60, '#', [[18, 'LL'], [34, 'LLLLL'], [52, 'LL']])],              // 12 床
    ugRows(2, 60, '#', [[18, 'LL'], [34, 'LLLLL'], [52, 'LL']])
);

// ── 部屋3「灰の昇り」 縦(登) 32タイル×36行【低1180 → 中508】──────────
// ⚠672pxだけ登る短い縦の部屋。段は3行(96px)刻みで右へ3タイルずつ。
// ⚠底は列10以降が溶岩＝踏み外すとチェックポイント復帰。動く床2枚（どちらも直後は静止した足場）。
var UG35_ROOM_RISE = [].concat(
    ugRows(2, 32, '#'),                                                     // 0-1  天井
    ugRows(10, 32, ' ', [[0, '#']]),                                        // 2-11 右は開放（鉱脈の道へ）
    [ugRow(32, ' ', [[0, '#'], [26, '######']])],                           // 12   出口の棚（中508）
    [ugRow(32, ' ', [[0, '#'], [25, 'o'], [29, '###']])],                   // 13
    [ugRow(32, ' ', [[0, '#'], [22, '^'], [31, '#']])],                     // 14   トゲ（C6の上）
    [ugRow(32, ' ', [[0, '#'], [21, '====='], [31, '#']])],                 // 15   C6
    [ugRow(32, ' ', [[0, '#'], [26, 'v'], [31, '#']])],                     // 16
    [ugRow(32, ' ', [[0, '#'], [19, '1'], [31, '#']])],                     // 17   回復（C5の真上）
    [ugRow(32, ' ', [[0, '#'], [18, 'MMMM'], [31, '#']])],                  // 18   C5＝動く床
    [ugRow(32, ' ', [[0, '#'], [24, 'F'], [31, '#']])],                     // 19   バー
    [ugRow(32, ' ', [[0, '#'], [16, '^^'], [31, '#']])],                    // 20   トゲ（C4の上）
    [ugRow(32, ' ', [[0, '#'], [15, '====='], [31, '#']])],                 // 21   C4 ⚠動く床の直後は静止
    [ugRow(32, ' ', [[0, '#'], [22, 'd'], [31, '#']])],                     // 22
    // ⚠行23-32は**左壁を開ける**（＝燠火の原の空間 860〜1148）
    [ugRow(32, ' ', [[14, 'o'], [31, '#']])],                               // 23   コイン（C3の真上）
    [ugRow(32, ' ', [[12, 'MMMM'], [31, '#']])],                            // 24   C3＝動く床
    [ugRow(32, ' ', [[18, 'G'], [31, '#']])],                               // 25   バー
    [ugRow(32, ' ', [[10, '^^'], [31, '#']])],                              // 26   トゲ（C2の上）
    [ugRow(32, ' ', [[9, '====='], [31, '#']])],                            // 27   C2 ⚠動く床の直後は静止
    [ugRow(32, ' ', [[16, 'v'], [31, '#']])],                               // 28
    [ugRow(32, ' ', [[8, 'o'], [31, '#']])],                                // 29   コイン（C1の真上）
    [ugRow(32, ' ', [[6, '====='], [31, '#']])],                            // 30   C1
    [ugRow(32, ' ', [[12, 'F'], [31, '#']])],                               // 31   バー
    [ugRow(32, ' ', [[4, 'S'], [31, '#']])],                                // 32   底のシャレコ
    ugRows(3, 32, '#', [[10, ugRep('L', 22)]])                              // 33-35 床 1180（列10以降は溶岩）
);

// ── 部屋4「鉱脈の道」 横 40タイル×15行【中層 508】────────────────────
// 分水嶺の直前。⚠ここでリズムを作ってから3分岐に入る（いきなり選ばせない）。
var UG35_ROOM_VEINS = [].concat(
    ugRows(2, 40, '#'),                                                     // 0-1 天井
    ugRows(3, 40, ' '),                                                     // 2-4
    [ugRow(40, ' ', [[12, 'H'], [30, 'H']])],                               // 5 天井バー
    ugRows(2, 40, ' '),                                                     // 6-7
    [ugRow(40, ' ', [[8, 'o'], [22, 'o'], [34, 'o']])],                     // 8
    [ugRow(40, ' ', [[16, '====='], [28, '=====']])],                       // 9 逃げ場
    [ugRow(40, ' ', [[18, 'o'], [26, '4']])],                               // 10
    [ugRow(40, ' ', [[4, 'm'], [6, 'F'], [10, '^^'], [14, 'v'], [17, 'e'], [21, 'S'],
                     [23, 'G'], [26, '^^'], [31, 'm'], [33, 'f'], [37, 'v'], [38, 'F']])], // 11
    [ugRow(40, '#', [[17, 'LL'], [33, 'LL']])],                             // 12 床
    ugRows(2, 40, '#', [[17, 'LL'], [33, 'LL']])
);
// ── 部屋5「分水嶺」 縦 176タイル×57行【中508 →（上-164 / 中508 / 低1180 の3並走）】★この面の看板
// ⚠**176列×57行は手で書けない**（R28で確認済み）。**岩で全部埋めてから通路を彫る**方式で組む。
// ⚠3本は**岩盤で完全に仕切る**: 上の床(行12)の下は行13-22が岩／中の床(行33)の下は行34-43が岩。
//   ＝**並走区間では絶対にルートを移れない**（R28で確立した約束を3本に広げただけ）。
// ⚠分かれ目は列40〜63の「分水嶺」だけ。中段を歩いてきて:
//     ・そのまま右へ    → 中ルート「渡り」
//     ・階段を登る       → 上ルート「梁の道」（1段目だけ4行=128px・以降3行=96px）
//     ・列40-41の穴へ降りる → 下ルート「窯の底」（穴の96px下に受け皿の段があるので落ちる先が見える）
// ⚠上ルートには**溶岩も穴も置かない**。行13-22の岩は仕切りなので、深いマグマを掘ると下ルートへ
//   抜けてしまう（R28では448px掘ったが、あれは仕切りが厚かったから成立していた）。
//   上の難しさは「バーと飛行敵の濃さ」で作る＝落ちないが確実に削られる道にする。
function ug35BuildDivide() {
    var W = 176, ROWS = 57, R_HI = 12, R_MID = 33, R_LOW = 54, r, c, i;
    // 上り階段 [段の行, 開始列, 終了列]（中33 → 高12）。⚠1段目だけ4行(128px)＝中段の頭上に
    //   96pxの隙間を残すため（3行だと段の下をくぐれない）
    var UP = [[29, 42, 44], [26, 45, 47], [23, 48, 50], [20, 51, 53], [17, 54, 56], [14, 57, 59], [12, 60, 63]];
    // 下り階段（中33 → 低54）。⚠穴(列40-41)のすぐ下＝行36が受け皿。以降3行(96px)刻み
    var DN = [[36, 40, 42], [39, 43, 45], [42, 46, 48], [45, 49, 51], [48, 52, 54], [51, 55, 57], [54, 58, 63]];
    var at = function (tbl, cc) {
        for (var k = 0; k < tbl.length; k++) if (cc >= tbl[k][1] && cc <= tbl[k][2]) return tbl[k][0];
        return null;
    };
    var g = [];
    for (r = 0; r < ROWS; r++) { g.push(new Array(W)); for (c = 0; c < W; c++) g[r][c] = '#'; }
    ...
    for (c = 0; c < W; c++) {
        if (c <= 39) { carve(23, 32, c); continue; }              // 入口＝中段の通路（床は行33）
        if (c <= 41) { carve(23, 35, c); continue; }              // ★分岐の穴（2タイル・ここだけ下へ抜ける）
        if (c <= 63) { carve(2, 32, c); carve(34, 53, c); continue; }  // ★分水嶺＝上下に大きくあける
        carve(2, 11, c);                                          // 上ルート（床 行12）
        carve(23, 32, c);                                         // 中ルート（床 行33）
        carve(44, 53, c);                                         // 下ルート（床 行54）
    }
    // ⚠**階段は片道足場('=')で作る**。岩の棚だと**真上に張り出した棚には乗れない**
    //   （頭をぶつけて終わり＝中段から上ルートへ上がれない）。片道足場なら下から突き抜けて乗れる。
    //   ⚠これは分水嶺（列40〜63）の中だけ＝**並走に入る前なら気が変わってもよい**、という作り。
    for (i = 0; i < UP.length; i++)
        for (c = UP[i][1]; c <= UP[i][2]; c++)
            if (UP[i][0] > R_HI) g[UP[i][0]][c] = '=';
    for (i = 0; i < DN.length; i++)
        for (c = DN[i][1]; c <= DN[i][2]; c++)
            if (DN[i][0] < R_LOW) g[DN[i][0]][c] = '=';
    // 上ルートへの取り付き（列60-63）は**岩**に戻す＝ここから先は仕切りの始まり（並走区間へ）
    for (c = 60; c <= 63; c++) for (r = R_HI; r <= 22; r++) g[r][c] = '#';
    // ── 飾り・ギミック・敵 [行, 列, 文字] ──
    var deco = [
        // 【入口の中段】バー2・トゲ2組・敵6・コイン3
        [32, 10, 'F'], [32, 26, 'G'], [32, 16, '^'], [32, 17, '^'], [32, 30, '^'], [32, 31, '^'],
        [29, 8, 'o'], [29, 22, 'o'], [29, 36, 'o'], [30, 18, '1'],
        [32, 4, 'm'], [32, 13, 'S'], [32, 20, 'v'], [32, 24, 'm'], [32, 34, 'S'], [32, 38, 'v'],
        // ★分かれ目の道しるべ。⚠**3本とも「道だ」と絵で分かるようにする**（コインで示す）
        [34, 40, 'o'], [35, 41, 'o'],                              // 下へ（穴の中）
        [28, 43, 'o'], [25, 46, 'o'], [22, 49, 'o'], [19, 52, 'o'], [16, 55, 'o'], [13, 58, 'o'], // 上へ（階段）
        [32, 50, 'o'], [32, 58, 'o'],                              // まっすぐ（中段）
        // 【上ルート＝梁の道】穴も溶岩も無い代わりにバーと飛行敵を最も濃く
        [3, 70, 'H'], [3, 92, 'H'], [3, 114, 'H'], [3, 136, 'H'], [3, 158, 'H'],
        [11, 66, 'F'], [11, 84, 'G'], [11, 102, 'F'], [11, 120, 'G'], [11, 140, 'F'], [11, 166, 'G'],
        [11, 74, '^'], [11, 75, '^'], [11, 96, '^'], [11, 97, '^'], [11, 110, '^'], [11, 111, '^'],
        [11, 128, '^'], [11, 129, '^'], [11, 148, '^'], [11, 149, '^'], [11, 170, '^'], [11, 171, '^'],
        [8, 68, 'o'], [8, 88, 'o'], [8, 108, 'o'], [8, 132, 'o'], [8, 154, 'o'], [8, 174, 'o'],
        [9, 90, '4'], [9, 130, '1'],
        [11, 64, 'm'], [11, 71, 'S'], [11, 80, 'm'], [11, 94, 'S'], [11, 106, 'm'], [11, 118, 'S'],
        [11, 134, 'm'], [11, 146, 'S'], [11, 160, 'm'], [11, 174, 'S'],
        [8, 78, 'v'], [8, 100, 'd'], [8, 116, 'v'], [8, 138, 'd'], [8, 152, 'v'], [8, 168, 'd'],
        [5, 82, 'v'], [5, 124, 'd'], [5, 162, 'v'],
        // 【中ルート＝渡り】動く床5枚＋2タイルの跳び越え5。実入り（コイン/アイテム）を最も多くする
        [32, 68, 'M'], [32, 69, 'M'], [32, 70, 'M'],
        [32, 88, 'M'], [32, 89, 'M'], [32, 90, 'M'],
        [32, 110, 'M'], [32, 111, 'M'], [32, 112, 'M'],
        [32, 132, 'M'], [32, 133, 'M'], [32, 134, 'M'],
        [32, 154, 'M'], [32, 155, 'M'], [32, 156, 'M'],
        [32, 78, 'e'], [32, 100, 'f'], [32, 122, 'e'], [32, 144, 'f'], [32, 166, 'e'],
        [32, 66, 'F'], [32, 96, 'G'], [32, 126, 'F'], [32, 152, 'G'],
        [32, 74, '^'], [32, 75, '^'], [32, 105, '^'], [32, 106, '^'], [32, 117, '^'], [32, 118, '^'],
        [32, 139, '^'], [32, 140, '^'], [32, 161, '^'], [32, 162, '^'],
        [29, 69, 'o'], [29, 78, 'o'], [29, 89, 'o'], [29, 100, 'o'], [29, 111, 'o'],
        [29, 122, 'o'], [29, 133, 'o'], [29, 144, 'o'], [29, 155, 'o'], [29, 166, 'o'],
        [30, 84, '1'], [30, 108, '4'], [30, 148, '3'],
        [31, 70, 'd'], [31, 90, 'v'], [31, 112, 'd'], [31, 134, 'v'], [31, 156, 'd'],
        [32, 73, 'm'], [32, 84, 'S'], [32, 103, 'm'], [32, 128, 'S'], [32, 159, 'm'], [32, 172, 'S'],
        // 【下ルート＝窯の底】敵とトゲが最も多い。噴出口もここに集める
        [45, 72, 'H'], [45, 100, 'H'], [45, 128, 'H'], [45, 156, 'H'],
        [53, 68, 'F'], [53, 88, 'G'], [53, 112, 'F'], [53, 132, 'G'], [53, 152, 'F'], [53, 170, 'G'],
        [53, 76, '^'], [53, 77, '^'], [53, 94, '^'], [53, 95, '^'], [53, 114, '^'], [53, 115, '^'],
        [53, 138, '^'], [53, 139, '^'], [53, 160, '^'], [53, 161, '^'],
        [53, 82, 'e'], [53, 120, 'f'], [53, 146, 'e'],
        [53, 104, 'M'], [53, 105, 'M'], [53, 106, 'M'],
        [50, 70, 'o'], [50, 92, 'o'], [50, 112, 'o'], [50, 136, 'o'], [50, 164, 'o'],
        [51, 86, '1'], [51, 126, '4'],
        [53, 64, 'm'], [53, 66, 'S'], [53, 71, 'm'], [53, 79, 'S'], [53, 86, 'm'], [53, 90, 'S'],
        [53, 98, 'm'], [53, 102, 'S'], [53, 110, 'm'], [53, 117, 'S'], [53, 124, 'm'], [53, 130, 'S'],
        [53, 135, 'm'], [53, 142, 'S'], [53, 149, 'm'], [53, 154, 'S'], [53, 164, 'm'], [53, 168, 'S'],
        [50, 74, 'v'], [50, 96, 'd'], [50, 118, 'v'], [50, 144, 'd'], [50, 172, 'v']
    ];
    for (i = 0; i < deco.length; i++) g[deco[i][0]][deco[i][1]] = deco[i][2];
    // ── 溶岩 [行の始まり, 行の終わり, 列の始まり, 列の終わり] ──
    // ⚠**中ルートの溶岩は行33-35の3行だけ**（行36-43の岩＝仕切りを必ず残す）。
    // ⚠**上ルートには一切置かない**（仕切りが10行しかないので掘れない＝落ちられてしまう）。
    var LAVA = [
        // 中ルート「渡り」: 動く床の下(4タイル)と跳び越え(2タイル)
        [33, 35, 68, 71], [33, 35, 78, 79], [33, 35, 88, 91], [33, 35, 100, 101],
        [33, 35, 110, 113], [33, 35, 122, 123], [33, 35, 132, 135], [33, 35, 144, 145],
        [33, 35, 154, 157], [33, 35, 166, 167],
        // 下ルート「窯の底」: 噴出口の池と動く床の池
        [54, 56, 82, 83], [54, 56, 104, 107], [54, 56, 120, 121], [54, 56, 146, 147]
    ];
    for (i = 0; i < LAVA.length; i++)
        for (r = LAVA[i][0]; r <= LAVA[i][1]; r++)
            for (c = LAVA[i][2]; c <= LAVA[i][3]; c++) g[r][c] = 'L';
...
var UG35_ROOM_DIVIDE = ug35BuildDivide();
// ── 部屋6「合流の大広間」 縦(降) 54タイル×57行【3本 → 低1180】────────────
// ⚠**descend:true**（カメラが足元の250px下まで映す）。3本が1本に戻る所なので、
//   どの高さから来ても**次の段が必ず画面に入る**ようにする（1.619〜1.620でR28が実機で不可になった箇所）。
// 合わせ方: 上ルート(行12) → 段16/22/28 → **中ルートの床(行33)に合流** → 段34/40/47 → 低の床(行54)。
//   下ルートは行54をそのまま歩いて来るので、何もしなくても合流する。
// ⚠段の高さは 128/192/192/160/32/192/224/224px＝すべて250px以内。
function ug35BuildConverge() {
    var W = 54, ROWS = 57, r, c, i;
    // 合流の段 [段の行, 開始列, 終了列]（3行厚）
    var STEP = [[16, 8, 10], [22, 12, 14], [28, 16, 18], [34, 20, 22], [40, 24, 26], [47, 28, 30]];
    var g = [];
    for (r = 0; r < ROWS; r++) { g.push(new Array(W)); for (c = 0; c < W; c++) g[r][c] = '#'; }
    var carve = function (r0, r1, col) {
        for (var rr = Math.max(2, r0); rr <= Math.min(ROWS - 4, r1); rr++) g[rr][col] = ' ';
    ...
    for (c = 0; c < W; c++) {
        carve(2, 53, c);                                   // まず大広間として全部あける
        if (c <= 7) for (r = 12; r <= 14; r++) g[r][c] = '#';    // 上ルートの入口の棚（-164）
        if (c <= 19) for (r = 33; r <= 35; r++) g[r][c] = '#';   // 中ルートの入口の床（508）＝合流点
    }
    for (i = 0; i < STEP.length; i++)                      // 降りる段を岩で戻す
        for (c = STEP[i][1]; c <= STEP[i][2]; c++)
            for (r = STEP[i][0]; r < STEP[i][0] + 3; r++) g[r][c] = '#';
    var deco = [
        // 段の上のコイン（⚠必ず段の真上の列に置く。外すと取れない）
        [15, 9, 'o'], [21, 13, 'o'], [27, 17, 'o'], [33, 21, 'o'], [39, 25, 'o'], [46, 29, 'o'],
        [11, 4, 'o'], [32, 12, 'o'], [32, 6, '1'],
        // 下ルート（行54）はそのまま通る＝ここで最後の詰め
        [53, 6, 'm'], [53, 14, 'S'], [53, 22, 'v'], [53, 34, 'm'], [53, 40, 'S'], [53, 50, 'v'],
        [53, 18, '^'], [53, 19, '^'], [53, 36, '^'], [53, 37, '^'],
        [53, 10, 'F'], [53, 30, 'G'], [45, 38, 'H'],
        [50, 34, 'o'], [50, 48, 'o'], [51, 42, '4']
    ];
    for (i = 0; i < deco.length; i++) g[deco[i][0]][deco[i][1]] = deco[i][2];
    // 大滝（'w'・**演出専用**／当たり判定なし）。⚠**この列には他のものを一切置かない**
    //   （1マスでも被るとその行だけ滝が抜けて2本に割れる＝R28の1.621で実際に起きた）
    for (r = 2; r <= 53; r++) for (c = 44; c <= 45; c++) if (g[r][c] === ' ') g[r][c] = 'w';
    var out = [];
    for (r = 0; r < ROWS; r++) out.push(g[r].join(''));
    return out;
}
var UG35_ROOM_CONVERGE = ug35BuildConverge();

// ── 部屋7「合流の底」 横 48タイル×15行【低層 1180】────────────────────
var UG35_ROOM_CONFLUX = [].concat(
    ugRows(2, 48, '#'),                                                     // 0-1 天井
    ugRows(3, 48, ' '),                                                     // 2-4
    [ugRow(48, ' ', [[12, 'H'], [34, 'H']])],                               // 5 天井バー
    ugRows(2, 48, ' '),                                                     // 6-7
    [ugRow(48, ' ', [[10, 'o'], [24, 'o'], [30, '1'], [40, 'o']])],         // 8
    [ugRow(48, ' ', [[18, '====='], [36, '=====']])],                       // 9 逃げ場
    [ugRow(48, ' ', [[14, 'o'], [44, 'o']])],                               // 10
    [ugRow(48, ' ', [[4, 'm'], [6, 'F'], [12, '^^'], [16, 'S'], [20, 'v'], [22, 'G'],
                     [26, '^^'], [30, 'm'], [33, 'e'], [37, 'S'], [39, 'F'],
                     [42, '^^'], [46, 'v']])],                              // 11
    [ugRow(48, '#', [[33, 'LL']])],                                         // 12 床
    ugRows(2, 48, '#', [[33, 'LL']])
);

// ── 部屋8「灰の梯子」 縦(登) 32タイル×57行【低1180 → 高-164】──────────
var UG35_ROOM_LADDER = [].concat(
    ugRows(2, 32, '#'),                                                     // 0-1  天井
    ugRows(10, 32, ' ', [[0, '#']]),                                        // 2-11 右は開放（玄室へ）
    [ugRow(32, ' ', [[0, '#'], [26, '######']])],                           // 12   出口の棚（-164）
    ugRows(2, 32, ' ', [[0, '#'], [29, '###']]),                            // 13-14
    [ugRow(32, ' ', [[0, '#'], [21, '====='], [31, '#']])],                 // 15   L11
    [ugRow(32, ' ', [[0, '#'], [26, 'v'], [31, '#']])],                     // 16
    [ugRow(32, ' ', [[0, '#'], [21, '1'], [31, '#']])],                     // 17   回復（L10の真上）
    [ugRow(32, ' ', [[0, '#'], [20, 'MMMM'], [31, '#']])],                  // 18   L10＝動く床
    [ugRow(32, ' ', [[0, '#'], [25, 'd'], [31, '#']])],                     // 19
    [ugRow(32, ' ', [[0, '#'], [23, 'G'], [31, '#']])],                     // 20   バー
    [ugRow(32, ' ', [[0, '#'], [19, 'o'], [31, '#']])],                     // 21   コイン（L9の真上）
    [ugRow(32, ' ', [[0, '#'], [18, '====='], [31, '#']])],                 // 22   L9
    [ugRow(32, ' ', [[0, '#'], [31, '#']])],                                // 23
    [ugRow(32, ' ', [[0, '#'], [26, 'v'], [31, '#']])],                     // 24
    [ugRow(32, ' ', [[0, '#'], [17, 'MMMM'], [31, '#']])],                  // 25   L8＝動く床
    [ugRow(32, ' ', [[0, '#'], [27, 'd'], [31, '#']])],                     // 26
    [ugRow(32, ' ', [[0, '#'], [20, 'F'], [31, '#']])],                     // 27
    [ugRow(32, ' ', [[0, '#'], [16, 'o'], [31, '#']])],                     // 28   コイン（L7の真上）
    [ugRow(32, ' ', [[0, '#'], [15, '====='], [31, '#']])],                 // 29   L7
    [ugRow(32, ' ', [[0, '#'], [24, 'v'], [31, '#']])],                     // 30
    [ugRow(32, ' ', [[0, '#'], [14, '^^'], [31, '#']])],                    // 31   トゲ（L6の上）
    [ugRow(32, ' ', [[0, '#'], [13, '====='], [31, '#']])],                 // 32   L6
    [ugRow(32, ' ', [[0, '#'], [22, 'd'], [31, '#']])],                     // 33
    [ugRow(32, ' ', [[0, '#'], [17, 'G'], [31, '#']])],                     // 34   バー
    [ugRow(32, ' ', [[0, '#'], [12, '4'], [31, '#']])],                     // 35   エナジー（L5の真上）
    [ugRow(32, ' ', [[0, '#'], [11, 'MMMM'], [31, '#']])],                  // 36   L5＝動く床
    [ugRow(32, ' ', [[0, '#'], [20, 'v'], [31, '#']])],                     // 37
    [ugRow(32, ' ', [[0, '#'], [10, '^'], [31, '#']])],                     // 38   トゲ（L4の上）
    [ugRow(32, ' ', [[0, '#'], [9, '====='], [31, '#']])],                  // 39   L4
    [ugRow(32, ' ', [[0, '#'], [16, 'd'], [31, '#']])],                     // 40
    [ugRow(32, ' ', [[0, '#'], [14, 'F'], [31, '#']])],                     // 41   バー
    [ugRow(32, ' ', [[0, '#'], [8, 'o'], [31, '#']])],                      // 42   コイン（L3の真上）
    [ugRow(32, ' ', [[0, '#'], [7, 'MMMM'], [31, '#']])],                   // 43   L3＝動く床
    // ⚠行44-53は**左壁を開ける**（＝燠火の原の空間 860〜1148）
    [ugRow(32, ' ', [[18, 'v'], [31, '#']])],                               // 44
    [ugRow(32, ' ', [[31, '#']])],                                          // 45
    [ugRow(32, ' ', [[5, '====='], [31, '#']])],                            // 46   L2 ⚠動く床の直後は静止
    [ugRow(32, ' ', [[11, 'G'], [31, '#']])],                               // 47   バー
    [ugRow(32, ' ', [[6, 'o'], [31, '#']])],                                // 48
    [ugRow(32, ' ', [[4, '^'], [31, '#']])],                                // 49   トゲ（L1の上）
    [ugRow(32, ' ', [[3, '====='], [31, '#']])],                            // 50   L1
    [ugRow(32, ' ', [[13, 'F'], [31, '#']])],                               // 51   バー
    [ugRow(32, ' ', [[31, '#']])],                                          // 52
    [ugRow(32, ' ', [[4, 'S'], [31, '#']])],                                // 53   底のシャレコ
    ugRows(3, 32, '#', [[8, ugRep('L', 24)]])                               // 54-56 床（列8以降は溶岩）
);
// ── 部屋9「宝物庫」 横 72タイル×15行【高層 -164・最難関】────────────
// 3分岐を抜けた先の山の頂。天井バー3・床バー5・トゲ5組・動く床2・噴出口2。
var UG35_ROOM_VAULT = [].concat(
    ugRows(2, 72, '#'),                                                     // 0-1 天井
    ugRows(3, 72, ' '),                                                     // 2-4
    [ugRow(72, ' ', [[8, 'H'], [30, 'H'], [56, 'H']])],                     // 5 天井バー
    ugRows(2, 72, ' '),                                                     // 6-7
    [ugRow(72, ' ', [[14, 'o'], [34, 'o'], [44, '1'], [60, 'o']])],         // 8
    [ugRow(72, ' ', [[20, '====='], [48, '=====']])],                       // 9 逃げ場
    [ugRow(72, ' ', [[4, '4'], [22, 'o'], [50, 'o']])],                     // 10
    [ugRow(72, ' ', [[2, 'm'], [6, 'F'], [9, 'v'], [12, '^^'], [16, 'MMMM'], [24, 'S'],
                     [26, 'G'], [28, 'm'], [31, '^^'], [36, 'f'], [40, '^^'], [44, 'm'],
                     [46, 'F'], [50, 'S'], [53, '^^'], [58, 'MMMM'], [64, 'v'], [66, 'G'],
                     [68, '^^'], [71, 'm']])],                              // 11
    [ugRow(72, '#', [[16, 'LLLL'], [35, 'LL'], [58, 'LLLL']])],             // 12 床
    ugRows(2, 72, '#', [[16, 'LLLL'], [35, 'LL'], [58, 'LLLL']])
);

// ── 部屋11「門前」 横 44タイル×15行【低層 1180】───────────────────────
// 参道の手前の小部屋。⚠ここで一度リズムを緩める（この先は店＝買い物の間）。
var UG35_ROOM_APPROACH = [].concat(
    ugRows(2, 44, '#'),                                                     // 0-1 天井
    ugRows(3, 44, ' '),                                                     // 2-4
    [ugRow(44, ' ', [[10, 'H'], [34, 'H']])],                               // 5 天井バー
    ugRows(2, 44, ' '),                                                     // 6-7
    [ugRow(44, ' ', [[8, 'o'], [22, 'o'], [30, '1']])],                     // 8
    [ugRow(44, ' ', [[14, '====='], [32, '=====']])],                       // 9 逃げ場
    [ugRow(44, ' ', [[18, 'o'], [38, 'o']])],                               // 10
    [ugRow(44, ' ', [[4, 'm'], [6, 'F'], [12, '^^'], [16, 'S'], [21, 'e'], [25, 'v'],
                     [27, 'G'], [30, '^^'], [35, 'm'], [39, 'F'], [42, 'S']])], // 11
    [ugRow(44, '#', [[21, 'LL']])],                                         // 12 床
    ugRows(2, 44, '#', [[21, 'LL']])
);

// ── 部屋10「顎の縦坑」 縦(降) 32タイル×57行【高-164 → 低1180】───────────
var UG35_ROOM_GULLET = [].concat(
    ugRows(2, 32, '#'),                                                     // 0-1  天井
    ugRows(10, 32, ' ', [[31, '#']]),                                       // 2-11 左は開放（宝物庫から入る）
    [ugRow(32, ' ', [[0, '#######'], [31, '#']])],                          // 12   入口の棚（-164）
    ugRows(2, 32, ' ', [[0, '###'], [31, '#']]),                            // 13-14
    [ugRow(32, ' ', [[0, '#'], [6, '^'], [31, '#']])],                      // 15   トゲ（D1の上）
    [ugRow(32, ' ', [[0, '#'], [4, '====='], [31, '#']])],                  // 16   D1
    [ugRow(32, ' ', [[0, '#'], [12, 'F'], [31, '#']])],                     // 17   バー
    [ugRow(32, ' ', [[0, '#'], [31, '#']])],                                // 18
    [ugRow(32, ' ', [[0, '#'], [14, 'G'], [24, 'v'], [31, '#']])],          // 19   バー
    [ugRow(32, ' ', [[0, '#'], [8, 'o'], [31, '#']])],                      // 20   コイン（D2の上）
    [ugRow(32, ' ', [[0, '#'], [6, 'MMMM'], [31, '#']])],                   // 21   D2＝動く床
    ugRows(2, 32, ' ', [[0, '#'], [31, '#']]),                              // 22-23
    [ugRow(32, ' ', [[0, '#'], [10, '^^'], [13, 'S'], [31, '#']])],         // 24   トゲ＋シャレコ（D3の上）
    [ugRow(32, ' ', [[0, '#'], [9, '====='], [31, '#']])],                  // 25   D3 ⚠動く床の直後は静止
    [ugRow(32, ' ', [[0, '#'], [22, 'd'], [31, '#']])],                     // 26
    [ugRow(32, ' ', [[0, '#'], [31, '#']])],                                // 27
    [ugRow(32, ' ', [[0, '#'], [18, 'F'], [31, '#']])],                     // 28   バー
    [ugRow(32, ' ', [[0, '#'], [13, 'o'], [31, '#']])],                     // 29   コイン（D4の上）
    [ugRow(32, ' ', [[0, '#'], [11, 'MMMM'], [31, '#']])],                  // 30   D4＝動く床
    ...
    [ugRow(32, ' ', [[0, '#'], [27, 'd'], [31, '#']])],                     // 42
    [ugRow(32, ' ', [[0, '#'], [18, 'MMMM'], [31, '#']])],                  // 43   D7＝動く床
    // ⚠行44-53は**右壁を開ける**（＝門前の空間 860〜1148）
    [ugRow(32, ' ', [[0, '#'], [26, 'F']])],                                // 44   バー
    [ugRow(32, ' ', [[0, '#']])],                                           // 45
    [ugRow(32, ' ', [[0, '#'], [22, '^'], [24, '1']])],                     // 46   トゲ＋回復（D8の上）
    [ugRow(32, ' ', [[0, '#'], [20, '=====']])],                            // 47   D8 ⚠動く床の直後は静止
    ugRows(2, 32, ' ', [[0, '#']]),                                         // 48-49
    [ugRow(32, ' ', [[0, '#'], [26, 'o']])],                                // 50   コイン（D9の上）
    [ugRow(32, ' ', [[0, '#'], [24, '=====']])],                            // 51   D9 ⚠着地点(列24-31)の真上
    [ugRow(32, ' ', [[0, '#'], [28, 'S']])],                                // 52
...
// ── 部屋12「参道」 横 136タイル×15行【低層 1180】───────────────────────
var UG35_ROOM_SANCTUM = [].concat(
    ugRows(2, 136, '#'),                                                    // 0-1 天井
    ugRows(3, 136, ' '),                                                    // 2-4
    [ugRow(136, ' ', [[101, 'bbbb'], [109, 'bbbbbb'], [126, 'bbbbbbbbbb']])], // 5 門のまぐさ
    ugRows(2, 136, ' ', [[111, 'bb'], [134, 'bb']]),                        // 6-7 門柱
    [ugRow(136, ' ', [[38, 'o'], [39, 'o'], [40, 'o'], [86, 'o'], [87, 'o'],
                      [111, 'bb'], [134, 'bb']])],                          // 8
    [ugRow(136, ' ', [[111, 'bb'], [134, 'bb']])],                          // 9
    [ugRow(136, ' ', [[8, 'F'], [20, 'G'], [111, 'bb'], [134, 'bb']])],     // 10 店より手前のギミック①バー2本
    [ugRow(136, ' ', [[134, 'bb'], [19, '^^'], [4, 'S'], [12, 'm'], [29, 'W'],
                      [42, 'v'], [54, 'm'], [64, 'S'], [76, 'v'], [86, 'm'],
                      [121, 'I'],
                      [48, 'i'], [72, 'i'], [88, 'i'], [96, 'i'], [102, 'i'], [106, 'i'],
                      [110, 'i'], [113, 'i'], [127, 'i'], [129, 'i'], [131, 'i'], [133, 'i']])], // 11
    [ugRow(136, '#', [[14, 'f ']])],                                        // 12 床（ギミック②裂け目）
...
// R35以降の部屋の並び。⚠合計 24+60+32+40+176+54+48+32+72+32+44+136+40 = **790タイル**
// ⚠高度の並び = **低→中→（上/中/下の3並走）→低→高→低**。3分岐は divide（列256〜431）。
var UG35_LEVEL_ROOMS = [
    { key: 'plunge',     wT: 24,  topY: -548, map: UG35_ROOM_PLUNGE, descend: true }, // 天井 → 床 1180（行54）
    { key: 'emberfield', wT: 60,  topY:  796, map: UG35_ROOM_EMBERFIELD }, // 床 1180
    { key: 'rise',       wT: 32,  topY:  124, map: UG35_ROOM_RISE },     // 底 1180（行33）→ 棚 508（行12）
    { key: 'veins',      wT: 40,  topY:  124, map: UG35_ROOM_VEINS },    // 床 508（中層）
    { key: 'divide',     wT: 176, topY: -548, map: UG35_ROOM_DIVIDE },   // ★中508 →（上/中/下の3分岐）
    // ⚠descend:true ＝ここだけカメラが下を向く。上ルートと中ルートが段で降りてくるので、
    //   足元の96px下までしか映さない通常の framing では次の段が見えない（R28の1.620と同じ理由）。
    { key: 'converge',   wT: 54,  topY: -548, map: UG35_ROOM_CONVERGE, descend: true }, // 3本 → 低1180
    { key: 'conflux',    wT: 48,  topY:  796, map: UG35_ROOM_CONFLUX },  // 床 1180
    { key: 'ladder',     wT: 32,  topY: -548, map: UG35_ROOM_LADDER },   // 底 1180（行54）→ 棚 -164（行12）
    { key: 'vault',      wT: 72,  topY: -548, map: UG35_ROOM_VAULT },    // 床 -164（高層・最難関）
    { key: 'gullet',     wT: 32,  topY: -548, map: UG35_ROOM_GULLET, descend: true }, // 棚 -164 → 底 1180
    { key: 'approach',   wT: 44,  topY:  796, map: UG35_ROOM_APPROACH }, // 床 1180
    { key: 'sanctum',    wT: 136, topY:  796, map: UG35_ROOM_SANCTUM },  // 床 1180（店29=643／巨像121=735）
    { key: 'chamber',    wT: 40,  topY:  796, map: UG_ROOM_CHAMBER }     // 闘技場だけ共用
];

// そのラウンドで使う部屋の並びを返す（1.599 R14／1.610 R21／1.611 R28／1.734 R35）。⚠**総幅はどれも790タイル**なので、
// どれを選んでもカメラ走行(UG_TRAVEL_PX)も距離加算も変わらない＝ランキングの前提は不変。
// R7=1周目／R14=2周目／R21=3周目／R28=4周目／R35以降=5周目（6周目以降も当面R35を使い回す）。
function ugRoomsForRound(round) {
    if (round >= 35) return UG35_LEVEL_ROOMS;   // 5周目「灼熱の階梯」（1.734）
...
var TITLE_SHOP_UPGRADES = [
    ...
    { id: 'idol_piyo', nameKey: 'skin_idol', descKey: 'tshop_idol_desc', grantSkin: 'idol',
      icon: '', iconImg: 'images/skin_idol_idle.png', maxLevel: 1, prices: [200000], effectDesc: ['5秒ごと範囲'], effectDescEn: ['5s burst'] },
...
var ZUKAN_ENTRIES = [
    ...
    { id: 'item:skin_idol',     cat: 'item', nameKey: 'skin_idol',     descKey: 'tshop_idol_desc',      img: 'images/skin_idol_idle.png',   seenIf: function(gs){ return (gs.ownedSkins || []).indexOf('idol') >= 0; } },
