...
var TEST_START_AFTER_R6 = true;    // ⚠⚠**出荷前に必ず false へ戻す**（実機テストの時だけ true にする）
// どこから始めるか:
...
//   'speedcap'  … **敵速度の上限（TEST_ENEMY_SPEED_CAP）を体感するモード（1.624）**。
//              開始ラウンドは testSpeedCapRound() が上限値から自動で決める（1.625）。
//              選定条件: ①スクロールが上限6.0px/f に達しきっている（6,000m以降）＝画面上の接近速度が
//              本番と同じ ②アカバネ（R11〜）が出る ③地底でもカカシでもない**普通の地上ラウンド**
//              ＝雑魚との真っ向勝負がそのまま見られる。
var TEST_START_MODE = 'speedcap';
// 開始地点をさらに前進させるm数（モードの基準地点からの相対）。通常は0でよい。
var TEST_START_OFFSET_M = 0;
// 敵の速度倍率の頭打ち（テスト専用・1.624／1.625で4.9→7.0）。0=頭打ちなし＝製品と同じ挙動。
// ⚠**TEST_START_AFTER_R6 が true の時だけ効く**（enemySpeedMult）。ここに値が残っていても
...
// 飛行敵だけの上限（1.626）。⚠**地上と同じ倍率では速すぎる**（ユーザー実機報告
// 「空中の雑魚敵が速すぎて反応できない」）。原因は素の速度差で、同じ倍率を掛けると差がそのまま開く:
//   地上 base 0.5〜2.5px/f に対し **飛行 base 2.0〜4.0px/f**。
//   7.0倍時の実測 → 地上最速 17.5px/f（画面に現れてから触れるまで0.44秒）
//                  飛行最速 **28.0px/f（0.30秒）**＝人間の単純反応(約0.25秒)とほぼ同じ＝避けようがない。
// 4.0倍にすると飛行は 8.0〜16.0px/f・接近22.0・**0.47秒**＝地上の最速(0.44秒)よりわずかに緩い。
// 空の敵は「跳んだ先に居る」形で当たるので、地上と同値(4.35倍)ではなく少し余裕を持たせている。
// 0 にすると enemySpeedMult() と同じ＝地上と共通（＝1.625までの挙動）。
var TEST_ENEMY_SPEED_CAP_FLY = 4.0;
function flyingSpeedMult() {
    var m = enemySpeedMult();
    if (TEST_START_AFTER_R6 && TEST_ENEMY_SPEED_CAP_FLY > 0 && m > TEST_ENEMY_SPEED_CAP_FLY) m = TEST_ENEMY_SPEED_CAP_FLY;
    return m;
}
