...
var TEST_START_AFTER_R6 = false;   // ⚠⚠**出荷前に必ず false へ戻す**（2026-07-27・R14の実機テストのため true）
// どこから始めるか:
...
// そのラウンドで使う部屋の並びを返す（1.599）。⚠**総幅はどちらも790タイル**なので、
// どちらを選んでもカメラ走行(UG_TRAVEL_PX)も距離加算も変わらない＝ランキングの前提は不変。
// R7=1周目のレイアウト／R14以降=2周目のレイアウト（R21で3周目を足す場合はここに1行足すだけ）。
