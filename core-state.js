...
//   ①高度の並びを **低→高→中→高→低**（二山＋**中央の谷が中層**）にした。
//     R7=一山／R14=一山／R21=低中低高低のジグザグ／R28=中層から上下分岐、なので「谷が中層」は新しい。
//   ②看板は **「渡り」(causeway)**＝中層の溶岩の海を**動く床5枚と3つの跳び越え**だけで渡る区間。
//     これまで動く床は最大11枚・すべて補助的な使い方で、**床がほぼ無い区間**を作るのは初めて。
//   ③部屋数12（最多）。縦の部屋を5つ（落下1・登り2・降り2）持ち、上り下りそのものを主菜にする。
// ⚠密度は R28（バー42/火の玉14/敵97/トゲ31/動く床8）比 **+25%** を目安。
//   ⚠**避けにくさでは上げない**（周期・予告・当たり判定は1周目のまま）＝SPEC §5.4 の方針。
// ⚠合計 24+100+32+120+32+90+32+100+32+52+136+40 = **790タイル**（他の4種と同じ＝距離加算は不変）。
...
// ── 部屋2「燠火（おきび）の原」 横 100タイル×15行【低層 1180】────────────
// 着地してすぐの部屋。⚠**列6までは無風**にする（落下直後の事故死を防ぐ・R7の教訓）。
// 溶岩の池5つ（3タイルの跳び越え3・5タイルの動く床渡り2）＋天井バー4＋床バー6＋トゲ6組。
var UG35_ROOM_EMBERFIELD = [].concat(
    ugRows(2, 100, '#'),                                                    // 0-1 天井
    ugRows(3, 100, ' '),                                                    // 2-4
    [ugRow(100, ' ', [[22, 'H'], [58, 'H'], [92, 'H']])],                   // 5 天井バー
    ugRows(2, 100, ' '),                                                    // 6-7
    [ugRow(100, ' ', [[10, 'o'], [20, 'o'], [36, 'o'], [50, 'o'], [56, '1'],
                      [68, 'o'], [78, 'o'], [90, 'o']])],                   // 8
    [ugRow(100, ' ', [[14, '====='], [38, '====='], [64, '====='], [88, '=====']])], // 9 逃げ場
    [ugRow(100, ' ', [[16, 'o'], [24, '4'], [32, 'o'], [42, 'o'], [58, 'o'], [82, 'o'], [96, 'o']])], // 10
    // 11 床のギミック＋敵。⚠動く床は**溶岩の左3列**に置く（右端1列は跳び移る間）
    [ugRow(100, ' ', [[6, 'm'], [10, '^^'], [13, 'F'], [14, 'S'], [18, 'e'], [23, 'v'],
                      [26, '^^'], [28, 'm'], [30, 'G'], [35, 'MMMM'], [41, 'v'],
                      [44, '^^'], [46, 'S'], [48, 'F'], [50, 'm'], [53, 'f'], [57, 'v'],
                      [60, '^^'], [62, 'G'], [64, 'S'], [67, 'MMMM'], [74, 'm'],
                      [76, '^^'], [79, 'v'], [80, 'F'], [85, 'e'], [88, 'm'], [91, 'S'],
                      [92, '^^'], [95, 'v'], [96, 'G'], [98, 'm']])],
    [ugRow(100, '#', [[18, 'LL'], [34, 'LLLLL'], [52, 'LL'], [66, 'LLLLL'], [84, 'LL']])], // 12 床
    ugRows(2, 100, '#', [[18, 'LL'], [34, 'LLLLL'], [52, 'LL'], [66, 'LLLLL'], [84, 'LL']])
);

// ── 部屋3「灰の梯子」 縦(登) 32タイル×57行【低1180 → 高-164】──────────
// ⚠1,344pxを11段で登る。動く床4枚（R28の昇りは3枚）＋バー7本＋トゲ3組。
// ⚠**動く床の次は必ず静止した足場**にする（連続させると位相待ちで詰む）。
// ⚠トゲは必ず「1つ下の行に静止した足場がある」位置だけ（動く床の上には置かない＝床だけ動いて分離する）。
...
// ── 部屋4「玄室の回廊」 横 120タイル×15行【高層 -164】────────────────
// 一つ目の山の頂。天井バー6・床バー6・トゲ8組・動く床2・噴出口2・敵21。
var UG35_ROOM_GALLERY = [].concat(
    ugRows(2, 120, '#'),                                                    // 0-1 天井
    ugRows(3, 120, ' '),                                                    // 2-4
    [ugRow(120, ' ', [[10, 'H'], [44, 'H'], [78, 'H'], [112, 'H']])],       // 5 天井バー
    ugRows(2, 120, ' '),                                                    // 6-7
    [ugRow(120, ' ', [[16, 'o'], [40, 'o'], [52, '1'], [64, 'o'], [88, 'o'], [112, 'o']])], // 8
    [ugRow(120, ' ', [[22, '====='], [46, '====='], [70, '====='], [94, '=====']])], // 9 逃げ場
    [ugRow(120, ' ', [[6, '4'], [24, 'o'], [48, 'o'], [72, 'o'], [96, 'o']])], // 10
    // 11 床のギミック＋敵26
    [ugRow(120, ' ', [[4, 'm'], [8, 'F'], [12, 'v'], [14, '^^'], [18, 'MMMM'], [24, 'S'],
                      [26, 'm'], [28, '^^'], [30, 'G'], [34, 'v'], [38, 'm'], [42, '^^'],
                      [46, 'S'], [50, 'v'], [52, 'F'], [54, '^^'], [57, 'm'], [59, 'S'],
                      [63, 'f'], [66, 'v'], [68, '^^'], [72, 'm'], [74, 'G'], [78, 'S'],
                      [82, 'v'], [84, 'm'], [86, '^^'], [90, 'MMMM'], [98, 'S'],
                      [100, 'v'], [102, '^^'], [104, 'm'], [107, 'e'], [110, '^^'],
                      [112, 'S'], [114, 'G'], [116, 'v'], [118, 'm']])],
    [ugRow(120, '#', [[18, 'LLLL'], [62, 'LL'], [90, 'LLLL'], [106, 'LL']])], // 12 床
    ugRows(2, 120, '#', [[18, 'LLLL'], [62, 'LL'], [90, 'LLLL'], [106, 'LL']])
);

// ── 部屋5「灰の滑降」 縦(降) 32タイル×36行【高-164 → 中508】────────────
// ⚠**672pxだけ降りる短い縦坑**（従来の縦坑は1,344px）。段は3行(96px)刻み＝descend のカメラ(250px)に必ず入る。
// ⚠底は列0-15が溶岩・列16-31だけが着地点＝まっすぐ落ちると溶岩。段を右へ辿ること。
// ⚠火の玉は置かない（降下中は周期を読む余裕が無い＝避けにくさで難度を上げることになる）。
var UG35_ROOM_CHUTE = [].concat(
    ugRows(2, 32, '#'),                                                     // 0-1  天井
    ugRows(10, 32, ' ', [[31, '#']]),                                       // 2-11 左は開放（玄室から入る）
    [ugRow(32, ' ', [[0, '#######'], [31, '#']])],                          // 12   入口の棚（-164）
    [ugRow(32, ' ', [[0, '###'], [31, '#']])],                              // 13
    [ugRow(32, ' ', [[0, '#'], [6, '^'], [31, '#']])],                      // 14   トゲ（D1の上）
    [ugRow(32, ' ', [[0, '#'], [4, '====='], [31, '#']])],                  // 15   D1
    [ugRow(32, ' ', [[0, '#'], [14, 'G'], [31, '#']])],                     // 16   バー
    [ugRow(32, ' ', [[0, '#'], [8, 'o'], [31, '#']])],                      // 17   コイン（D2の上）
    [ugRow(32, ' ', [[0, '#'], [7, '====='], [31, '#']])],                  // 18   D2
    [ugRow(32, ' ', [[0, '#'], [20, 'v'], [31, '#']])],                     // 19
    [ugRow(32, ' ', [[0, '#'], [12, '^^'], [31, '#']])],                    // 20   トゲ（D3の上）
    [ugRow(32, ' ', [[0, '#'], [10, '====='], [31, '#']])],                 // 21   D3
    [ugRow(32, ' ', [[0, '#'], [18, 'F'], [31, '#']])],                     // 22   バー
    // ⚠行23-32は**右壁を開ける**（＝渡りの空間 188〜476）
    [ugRow(32, ' ', [[0, '#'], [16, 'o']])],                                // 23   コイン（D4の上）
    [ugRow(32, ' ', [[0, '#'], [13, 'MMMM']])],                             // 24   D4＝動く床
    [ugRow(32, ' ', [[0, '#'], [24, 'd']])],                                // 25
    [ugRow(32, ' ', [[0, '#'], [18, '4']])],                                // 26   エナジー（D5の上）
    [ugRow(32, ' ', [[0, '#'], [16, '=====']])],                            // 27   D5 ⚠動く床の直後は静止
    [ugRow(32, ' ', [[0, '#'], [22, 'G']])],                                // 28   バー
    [ugRow(32, ' ', [[0, '#'], [21, 'o']])],                                // 29   コイン（D6の上）
    [ugRow(32, ' ', [[0, '#'], [19, '=====']])],                            // 30   D6
    [ugRow(32, ' ', [[0, '#'], [26, 'S']])],                                // 31
    [ugRow(32, ' ', [[0, '#']])],                                           // 32
    ugRows(3, 32, '#', [[0, ugRep('L', 16)]])                               // 33-35 床 508（列0-15は溶岩）
);

// ── 部屋6「渡り」 横 90タイル×15行【中層 508】★この面の看板 ───────────
// 溶岩の海を**動く床5枚＋跳び越え3つ**だけで渡る。⚠落ちても死なない（チェックポイント復帰）が、
//   **足場がほぼ無い**＝これまでのどの部屋とも手触りが違う。動く床は溶岩の左3列に置き、
//   右端の1列は「跳び移る間」として空ける（＝乗り移りのリズムを毎回同じにする）。
// ⚠**動く床の上にトゲは置かない**／⚠噴出口は必ず溶岩の上（跳び越える3タイルの池の中央）。
// ⚠地上の敵は島の上だけに置く（溶岩の上に置くと落ちて消える）。溶岩の上は飛行敵('v'/'d')にする。
var UG35_ROOM_CAUSEWAY = [].concat(
    ugRows(2, 90, '#'),                                                     // 0-1 天井
    ugRows(3, 90, ' '),                                                     // 2-4
    [ugRow(90, ' ', [[18, 'H'], [60, 'H']])],                               // 5 天井バー
    ugRows(3, 90, ' '),                                                     // 6-8
    [ugRow(90, ' ', [[11, 'd'], [21, 'v'], [30, 'd'], [40, 'v'], [49, 'd'],
                     [59, 'v'], [69, 'd'], [78, 'v']])],                    // 9 溶岩の上を舞う飛行敵
    [ugRow(90, ' ', [[11, 'o'], [21, 'o'], [30, 'o'], [40, 'o'], [44, '1'],
                     [49, 'o'], [59, 'o'], [69, 'o'], [78, 'o'], [85, '4']])], // 10
    // 11 動く床・噴出口・トゲ・バー・地上敵
    [ugRow(90, ' ', [[3, 'm'], [6, 'F'], [10, 'MMM'], [15, 'S'], [16, '^^'], [21, 'e'],
                     [24, 'G'], [25, 'm'], [26, '^^'], [29, 'MMM'], [34, 'S'], [35, '^^'],
                     [40, 'f'], [43, 'F'], [45, '^^'], [48, 'MMM'], [53, 'm'], [54, '^^'],
                     [56, 'S'], [58, 'MMM'], [63, 'G'], [65, '^^'], [69, 'e'], [72, 'm'],
                     [73, '^^'], [77, 'MMM'], [82, 'S'], [83, 'F'], [86, '^^'], [88, 'm']])],
    [ugRow(90, '#', [[10, 'LLLL'], [20, 'LL'], [29, 'LLLL'], [39, 'LL'],
                     [48, 'LLLL'], [58, 'LLLL'], [68, 'LL'], [77, 'LLLL']])], // 12 床＝溶岩の海
    ugRows(2, 90, '#', [[10, 'LLLL'], [20, 'LL'], [29, 'LLLL'], [39, 'LL'],
                        [48, 'LLLL'], [58, 'LLLL'], [68, 'LL'], [77, 'LLLL']])
);

// ── 部屋7「熔骨の尖塔」 縦(登) 32タイル×36行【中508 → 高-164】───────────
// ⚠2つ目の山への登り。底は列10以降が溶岩＝踏み外すとチェックポイント復帰。
// ⚠段は3行(96px)刻みで右へ3タイルずつ。動く床2枚（どちらも直後は静止した足場）。
var UG35_ROOM_SPIRE = [].concat(
    ugRows(2, 32, '#'),                                                     // 0-1  天井
    ugRows(10, 32, ' ', [[0, '#']]),                                        // 2-11 右は開放（宝物庫へ）
    [ugRow(32, ' ', [[0, '#'], [26, '######']])],                           // 12   出口の棚（-164）
    ...
    [ugRow(32, ' ', [[0, '#'], [14, 'o'], [31, '#']])],                     // 23   コイン（C3の真上）
    // ⚠行24-32は**左壁を開ける**（＝渡りの空間 188〜476）
    ...
    ugRows(3, 32, '#', [[10, ugRep('L', 22)]])                              // 33-35 床 508（列10以降は溶岩）
);

// ── 部屋8「宝物庫」 横 100タイル×15行【高層 -164・最難関】────────────
// 二つ目の山の頂＝この面で最も密。天井バー5・床バー6・トゲ7組・動く床2・噴出口2・敵24。
var UG35_ROOM_VAULT = [].concat(
    ugRows(2, 100, '#'),                                                    // 0-1 天井
    ugRows(3, 100, ' '),                                                    // 2-4
    [ugRow(100, ' ', [[8, 'H'], [36, 'H'], [64, 'H'], [92, 'H']])],         // 5 天井バー
    ugRows(2, 100, ' '),                                                    // 6-7
    [ugRow(100, ' ', [[14, 'o'], [34, 'o'], [44, '1'], [54, 'o'], [74, 'o'], [94, 'o']])], // 8
    [ugRow(100, ' ', [[20, '====='], [60, '====='], [84, '=====']])],       // 9 逃げ場
    [ugRow(100, ' ', [[4, '4'], [22, 'o'], [40, 'o'], [62, 'o'], [86, 'o']])], // 10
    // 11 床のギミック＋敵24
    [ugRow(100, ' ', [[2, 'm'], [6, 'F'], [9, 'v'], [12, '^^'], [16, 'MMMM'], [24, 'S'],
                      [26, 'G'], [28, 'm'], [30, '^^'], [33, 'v'], [36, 'f'], [40, '^^'],
                      [44, 'm'], [46, 'F'], [50, 'S'], [52, '^^'], [56, 'MMMM'], [62, 'v'],
                      [64, 'm'], [66, 'G'], [70, '^^'], [73, 'S'], [76, 'e'], [78, '^^'],
                      [82, 'v'], [84, 'm'], [90, 'S'], [92, '^^'], [96, 'v'],
                      [98, 'G']])],
    [ugRow(100, '#', [[16, 'LLLL'], [35, 'LL'], [56, 'LLLL'], [75, 'LL']])], // 12 床
    ugRows(2, 100, '#', [[16, 'LLLL'], [35, 'LL'], [56, 'LLLL'], [75, 'LL']])
);

// ── 部屋9「顎の縦坑」 縦(降) 32タイル×57行【高-164 → 低1180】───────────
// ⚠底は列0-23が溶岩・列24-31だけが着地点。⚠火の玉は置かない（降下中は周期を読めない）。
// ⚠R28の同名部屋より動く床を1枚増やし（D2）、バーを2本足した。
...
// ── 部屋10「門前」 横 52タイル×15行【低層 1180】───────────────────────
// 参道の手前の小部屋。⚠ここで一度リズムを緩める（この先は店＝買い物の間）。
var UG35_ROOM_APPROACH = [].concat(
    ugRows(2, 52, '#'),                                                     // 0-1 天井
    ugRows(3, 52, ' '),                                                     // 2-4
    [ugRow(52, ' ', [[12, 'H'], [40, 'H']])],                               // 5 天井バー
    ugRows(2, 52, ' '),                                                     // 6-7
    [ugRow(52, ' ', [[8, 'o'], [24, 'o'], [34, '1'], [40, 'o']])],          // 8
    [ugRow(52, ' ', [[16, '====='], [36, '=====']])],                       // 9 逃げ場
    [ugRow(52, ' ', [[18, 'o'], [44, 'o']])],                               // 10
    [ugRow(52, ' ', [[4, 'm'], [6, 'F'], [12, '^^'], [14, 'S'], [21, 'e'], [24, 'G'],
                     [26, 'v'], [28, '^^'], [32, 'm'], [38, 'S'], [42, 'F'],
                     [48, '^^'], [50, 'v']])],                              // 11
    [ugRow(52, '#', [[20, 'LL']])],                                         // 12 床
    ugRows(2, 52, '#', [[20, 'LL']])
);

// ── 部屋11「参道」 横 136タイル×15行【低層 1180】───────────────────────
// ⚠店 = 開始614 + 列29 = **643タイル目**／巨像 = 614 + 列121 = **735タイル目**（R7/R14/R21/R28と同一）。
// ⚠店の入口(列27〜33)と門から先(列100〜)には何も置かない＝入店操作を邪魔しない／静けさを保つ。
// ⚠列117〜125には石を置かない＝巨像の正面を壁龕にする。
// ⚠この面だけ**店より手前のギミックを厚くする**（バー2＋トゲ2組＋裂け目）＝最難関の面らしく、
//   買い物にたどり着くまでにも手数を要る形にする。それでも店の前後は無風のまま。
...
// R35以降の部屋の並び。⚠合計 24+100+32+120+32+90+32+100+32+52+136+40 = **790タイル**
// ⚠高度の並び = **低→高→中→高→低**（二山・谷が中層）。
var UG35_LEVEL_ROOMS = [
    { key: 'plunge',     wT: 24,  topY: -548, map: UG35_ROOM_PLUNGE, descend: true }, // 天井 → 床 1180（行54）
    { key: 'emberfield', wT: 100, topY:  796, map: UG35_ROOM_EMBERFIELD },  // 床 1180
    { key: 'ladder',     wT: 32,  topY: -548, map: UG35_ROOM_LADDER },      // 底 1180（行54）→ 棚 -164（行12）
    { key: 'gallery',    wT: 120, topY: -548, map: UG35_ROOM_GALLERY },     // 床 -164（高層）
    { key: 'chute',      wT: 32,  topY: -548, map: UG35_ROOM_CHUTE, descend: true },  // 棚 -164 → 床 508（行33）
    { key: 'causeway',   wT: 90,  topY:  124, map: UG35_ROOM_CAUSEWAY },    // 床 508（中層）★渡り
    { key: 'spire',      wT: 32,  topY: -548, map: UG35_ROOM_SPIRE },       // 底 508（行33）→ 棚 -164（行12）
    { key: 'vault',      wT: 100, topY: -548, map: UG35_ROOM_VAULT },       // 床 -164（高層・最難関）
    { key: 'gullet',     wT: 32,  topY: -548, map: UG35_ROOM_GULLET, descend: true }, // 棚 -164 → 底 1180
    { key: 'approach',   wT: 52,  topY:  796, map: UG35_ROOM_APPROACH },    // 床 1180
