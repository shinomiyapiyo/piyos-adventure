...
//   'cap49'  … **敵速度の上限4.9倍を体感するモード（1.624）**。R17＝36,000mから開始。
//              ここを選ぶ理由: ①スクロールが上限6.0px/f に達しきっている（6,000m以降）＝画面上の
//              接近速度が本番と同じ ②アカバネ（R11〜）が出る ③R17は地底でもカカシでもない**普通の
//              地上ラウンド**なので、雑魚との真っ向勝負がそのまま見られる。
//              ⚠素の倍率は5.8倍。TEST_ENEMY_SPEED_CAP で4.9へ頭打ちにしている＝**同じ地点で
//                キャップ有り/無しを比べられる**（下の定数を0にすれば5.8倍のまま＝現状の挙動）。
var TEST_START_MODE = 'cap49';
// 開始地点をさらに前進させるm数（モードの基準地点からの相対）。通常は0でよい。
var TEST_START_OFFSET_M = 0;
// 敵の速度倍率の頭打ち（テスト専用・1.624）。0=頭打ちなし＝製品と同じ挙動。
// ⚠**TEST_START_AFTER_R6 が true の時だけ効く**（enemySpeedMult）。ここに値が残っていても
//   出荷ビルドでは無効＝消し忘れても製品の速度は変わらない。
// ⚠A/B比較の手順: 'cap49' で走る → この値を 0 にして同じ地点を走る（素の5.8倍）→ どちらが良いか判断。
var TEST_ENEMY_SPEED_CAP = 4.9;
// 'cap49' で開始するラウンド。R17=36,000m。⚠地底(R7/14/21/28)とカカシ(R6/13/20/27)を避けた普通のラウンド。
var TEST_SPEED_CAP_ROUND = 17;
