...
| | 状態 |
|---|---|
| **Android** | クローズドテスト Alpha に **20 (1.8.1)** を提出。公開の概要が「**審査中の変更**」に変わったのを確認。完全公開100% |
| **iOS** | **1.8.1 / build 22** を提出。「1項目が提出されました」＋サイドバーが「**1.8.1 審査待ち**」を確認。リリース方法＝**承認後に自動リリース**（`AFTER_APPROVAL`） |

### 入力した文面

- **Play のリリースノート**: 1つの textarea に `<ja-JP>` / `<en-US>` 併記。ja 342字 / en 472字（上限500）。
  「リリースノート: 2 言語」と認識されたのを確認
- **ASC このバージョンの最新情報**: ja 376字 / en 692字（上限4000）
- **ASC プロモーション用テキスト**: ja 100字 / en 135字（上限170）。**定型の売り文句のみ**（不具合修正は書かない）
- **ASC 審査メモ**: 2,383字。1.742 の内容に書き直し（和英併記）。横向き専用／ログイン不要／
  広告はNPAのみでATT非表示／ランキングのモデレーション／課金なし の定番は維持

### 今回踏んだ罠

- **ASC の作業中にブラウザペインが落ちた**（スクショ真っ白＋`document.visibilityState === 'hidden'`）。
  → **ネイティブsetterでの入力は通る**のでテキストだけ先に入れ、ユーザーにペインを広げてもらって再開した。
  ⚠この間 `.click()` での迂回はしない（[[piyo-browser-pane-hidden]]）
- **Play の言語切替と ASC の言語切替は別物**。ASC は 日本語 → 保存 → 英語 → 保存 と**言語ごとに保存**。
  保存できたかは**保存ボタンが disabled になったか**で判定した
- Play のクローズドテスト track へ**直URLで飛ぶとエラー**（`681956C1`）。
  テストとリリース → クローズド テスト → Alpha と**UIをたどる**のが確実
